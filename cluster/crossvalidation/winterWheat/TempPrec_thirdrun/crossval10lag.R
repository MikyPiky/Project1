#################################################################################
#### Cross Validation to determine the model of WinterWheat for October:lag ####
#################################################################################
...
Yield_Covariates_nna$SMI_Oct_lag_GDM <- cut(Yield_Covariates_nna$SMI_Oct_lag, breaks = c(0, 0.1, 0.2, 0.3, 0.7, 0.8, 0.9, 1),                                            ,
...
formula_PrecTavgSmiGDM_2v3poly<- log(winterWheat) ~ poly(Prec_Oct_lag, container_2v3poly[r, 1], raw = T) +  poly(Tavg_Oct_lag, container_2v3poly[r, 2], raw = T) + 
  dummy(SMI_Oct_lag_GDM,c("severe drought","moderate drought","abnormal dry", "abnormal wet", "abundant wet","severe wet")) + log(as.integer(year)) + 
  dummy(comId)
...
for(r in 1:9){
  ...
  save.image(file = "/home/peichl/projects/crossvalid/cross10lag.RData")
}
