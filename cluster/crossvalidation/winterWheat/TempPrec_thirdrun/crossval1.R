##############################################################################
#### Cross Validation to determine the model of WinterWheat for Januar ####
##############################################################################
...
Yield_Covariates_nna$SMI_Jan_GDM <- cut(Yield_Covariates_nna$SMI_Jan, breaks = c(0, 0.1, 0.2, 0.3, 0.7, 0.8, 0.9, 1),                                            ,
...
formula_PrecTavgSmiGDM_2v3poly<- log(winterWheat) ~ poly(Prec_Jan, container_2v3poly[r, 1], raw = T) +  poly(Tavg_Jan, container_2v3poly[r, 2], raw = T) + 
  dummy(SMI_Jan_GDM,c("severe drought","moderate drought","abnormal dry", "abnormal wet", "abundant wet","severe wet")) + log(as.integer(year)) + 
  dummy(comId)
