########################################################################
#### Cross Validation to determine the model of WinterWheat for Mai ####
########################################################################
...
formula_PrecTavgSmiGDM_2v3poly <- log(winterWheat) ~ poly(Prec_Mai, container_2v3poly[r, 1], raw = T) +  poly(Tavg_Mai, container_2v3poly[r, 2], raw = T) + 
...
for(r in 1:9){
  ...
  save.image(file = "/home/peichl/projects/crossvalid/cross5.RData")
}
...
BIC_loop_Mai_ww <- BIC_loop_2v8poly_Prec
cv.error_loocv_Mai_ww <- cv.error_loocv_2v8poly_Prec

par(mfrow=c(2,1))
plot(BIC_loop_2v8poly_Prec)
plot(cv.error_loocv_2v8poly_Prec)
plot(BIC_loop_Mai)
plot(cv.error_loocv_Mai)

which.min(BIC_loop_Mai_ww)
which.min(cv.error_loocv_Mai_ww) 


# ich denke, dass hier Model Nummer 3am besten ist
r_Mai_ww = which.min(BIC_loop_Mai_ww)
r = r_Mai_ww
...
Yield_Covariates_nna_plm$SMI_Mai_within <- Within(Yield_Covariates_nna_plm$SMI_Mai)
Yield_Covariates_nna_plm$Prec_Mai_within <- Within(Yield_Covariates_nna_plm$Prec_Mai)
Yield_Covariates_nna_plm$Prec_Mai_within <- Within(Yield_Covariates_nna_plm$Tavg_Mai)
Yield_Covariates_nna_plm$SMI_Mai_GDM_within <- Within(Yield_Covariates_nna_plm$SMI_Mai_GDM)
sumMaiy(Yield_Covariates_nna_plm$Prec_Mai$)
...
Yield_Covariates_Mai <- Yield_Covariates_nna_plm[,Mai]
names(Yield_Covariates_Mai)
Yield_Covariates_Mai$siloMaize <- NULL
dim(Yield_Covariates_Mai)

Yield_Covariates_ww <- Yield_Covariates_nna_plm[,1:7]
names(Yield_Covariates_ww)

Yield_Covariates_Mai_ww <- cbind(Yield_Covariates_ww, Yield_Covariates_Mai)
names(Yield_Covariates_Mai_ww)
head(Yield_Covariates_Mai_ww)
names(Yield_Covariates_Mai_ww) <- c("year","comId","com","comIdState","comState","SHAPE_AREA","winterWheat", "SMI","Prec",
...
## Determine formula
r <- r_Mai_ww
container_2v3poly[r_Mai_ww, 1]
container_2v3poly[r_Mai_ww, 2]

formula_Mai_ww <- log(winterWheat) ~ poly(Prec, container_2v3poly[r, 1], raw = T) +  poly(Tavg, container_2v3poly[r, 2], raw = T) + dummy(SMI_GDM,c("severe drought","moderate drought","abnormal dry", "abnormal wet", "abundant wet","severe wet")) + log(as.integer(year)) + 
  dummy(comId)

####################
## PLM Ergebnisse ##
Yield_Covariates_Mai_ww_plm <- pdata.frame(Yield_Covariates_Mai_ww, index = c("comId","year"))
head(Yield_Covariates_Mai_ww_plm)
plm.fit_Mai_ww <- plm(formula= update(formula_Mai_ww, .~. - dummy(comId)),  data = Yield_Covariates_Mai_ww_plm)
summary(plm.fit_Mai_ww)
