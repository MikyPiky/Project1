#########################################################################
#### Cross Validation to determine the model of WinterWheat for July ####
#########################################################################
...
formula_PrecTavgSmiGDM_2v3poly <- log(winterWheat) ~ poly(Prec_Jul, container_2v3poly[r, 1], raw = T) +  poly(Tavg_Jul, container_2v3poly[r, 2], raw = T) + 
...
for(r in 1:9){
  ...
  save.image(file = "/home/peichl/projects/crossvalid/cross7.RData")
}

## Load environment with data which has been produced on cluster
load("/home/peichl/Documents/projects/correlation/data/data_processed/Crossvalidation/winterWheat/cross7.RData")
...
BIC_loop_Jul_ww <- BIC_loop_2v8poly_Prec
cv.error_loocv_Jul_ww <- cv.error_loocv_2v8poly_Prec

par(mfrow=c(2,1))
plot(BIC_loop_2v8poly_Prec)
plot(cv.error_loocv_2v8poly_Prec)
plot(BIC_loop_Jul)
plot(cv.error_loocv_Jul)

which.min(BIC_loop_Jul_ww)
which.min(cv.error_loocv_Jul_ww) 


# ich denke, dass hier Model Nummer 3 am besten ist
r_Jul_ww = which.min(BIC_loop_Jul_ww)
r = r_Jul_ww
...
Yield_Covariates_nna_plm$SMI_Jul_within <- Within(Yield_Covariates_nna_plm$SMI_Jul)
Yield_Covariates_nna_plm$Prec_Jul_within <- Within(Yield_Covariates_nna_plm$Prec_Jul)
Yield_Covariates_nna_plm$Prec_Jul_within <- Within(Yield_Covariates_nna_plm$Tavg_Jul)
Yield_Covariates_nna_plm$SMI_Jul_GDM_within <- Within(Yield_Covariates_nna_plm$SMI_Jul_GDM)
sumJuny(Yield_Covariates_nna_plm$Prec_Jul$)
...
Yield_Covariates_Jul <- Yield_Covariates_nna_plm[,Jul]
...
Yield_Covariates_Jul_ww <- cbind(Yield_Covariates_ww, Yield_Covariates_Jul)
names(Yield_Covariates_Jul_ww)
head(Yield_Covariates_Jul_ww)
names(Yield_Covariates_Jul_ww) <- c("year","comId","com","comIdState","comState","SHAPE_AREA","winterWheat", "SMI","Prec",
...
## Determine formula
r <- r_Jul_ww
container_2v3poly[r_Jul_ww, 1]
container_2v3poly[r_Jul_ww, 2]

formula_Jul_ww_sm <- log(winterWheat) ~ poly(Prec, container_2v3poly[r, 1], raw = T) +  poly(Tavg, container_2v3poly[r, 2], raw = T) + dummy(SMI_GDM,c("severe drought","moderate drought","abnormal dry", "abnormal wet", "abundant wet","severe wet")) + log(as.integer(year)) + 
  dummy(comId)

####################
## PLM Ergebnisse ##
Yield_Covariates_Jul_ww_plm <- pdata.frame(Yield_Covariates_Jul_ww, index = c("comId","year"))
head(Yield_Covariates_Jul_ww_plm)
plm.fit_Jul_ww <- plm(formula= update(formula_Jul_ww_sm, .~. - dummy(comId)),  data = Yield_Covariates_Jul_ww_plm)
summary(plm.fit_Jul_ww)
