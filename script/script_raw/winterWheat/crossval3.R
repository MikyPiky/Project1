##########################################################################
#### Cross Validation to determine the model of WinterWheat for March ####
##########################################################################
...
Yield_Covariates_nna$SMI_Mar_GDM <- cut(Yield_Covariates_nna$SMI_Mar, breaks = c(0, 0.1, 0.2, 0.3, 0.7, 0.8, 0.9, 1),                                            ,
...
formula_PrecTavgSmiGDM_2v3poly <- log(winterWheat) ~ poly(Prec_Mar, container_2v3poly[r, 1], raw = T) +  poly(Tavg_Mar, container_2v3poly[r, 2], raw = T) + 
  dummy(SMI_Mar_GDM,c("severe drought","moderate drought","abnormal dry", "abnormal wet", "abundant wet","severe wet")) + log(as.integer(year)) + 
  dummy(comId)
...
for(r in 1:9){
  ...
  save.image(file = "/home/peichl/projects/crossvalid/cross3.RData")
}

## Load environment with data which has been produced on cluster
load("/home/peichl/Documents/projects/correlation/data/data_processed/Crossvalidation/winterWheat/cross3.RData")
...
BIC_loop_Mar_ww <- BIC_loop_2v8poly_Prec
cv.error_loocv_Mar_ww <- cv.error_loocv_2v8poly_Prec

par(mfrow=c(2,1))
plot(BIC_loop_2v8poly_Prec)
plot(cv.error_loocv_2v8poly_Prec)
plot(BIC_loop_Mar)
plot(cv.error_loocv_Mar,main="March", ylab="test MSE")

which.min(BIC_loop_Mar_ww)
which.min(cv.error_loocv_Mar_ww) 


# ich denke, dass hier Model Nummer 3 am besten ist
r_Mar_ww = which.min(BIC_loop_Mar_ww)
r = r_Mar_ww
...
Yield_Covariates_nna_plm$SMI_Mar_within <- Within(Yield_Covariates_nna_plm$SMI_Mar)
Yield_Covariates_nna_plm$Prec_Mar_within <- Within(Yield_Covariates_nna_plm$Prec_Mar)
Yield_Covariates_nna_plm$Prec_Mar_within <- Within(Yield_Covariates_nna_plm$Tavg_Mar)
Yield_Covariates_nna_plm$SMI_Mar_GDM_within <- Within(Yield_Covariates_nna_plm$SMI_Mar_GDM)
summary(Yield_Covariates_nna_plm$Prec_Mar$)
...
Yield_Covariates_Mar <- Yield_Covariates_nna_plm[,Mar]
...
Yield_Covariates_Mar_ww <- cbind(Yield_Covariates_ww, Yield_Covariates_Mar)
names(Yield_Covariates_Mar_ww)
head(Yield_Covariates_Mar_ww)
names(Yield_Covariates_Mar_ww) <- c("year","comId","com","comIdState","comState","SHAPE_AREA","winterWheat", "SMI","Prec",
...
## Determine formula
r <- r_Mar_ww
container_2v3poly[r_Mar_ww, 1]
container_2v3poly[r_Mar_ww, 2]

formula_Mar_ww <- log(winterWheat) ~ poly(Prec, container_2v3poly[r, 1], raw = T) +  poly(Tavg, container_2v3poly[r, 2], raw = T) + dummy(SMI_GDM,c("severe drought","moderate drought","abnormal dry", "abnormal wet", "abundant wet","severe wet")) + log(as.integer(year)) + 
  dummy(comId)

####################
## PLM Ergebnisse ##
Yield_Covariates_Mar_ww_plm <- pdata.frame(Yield_Covariates_Mar_ww, index = c("comId","year"))
head(Yield_Covariates_Mar_ww_plm)
plm.fit_Mar_ww <- plm(formula= update(formula_Mar_ww, .~. - dummy(comId)),  data = Yield_Covariates_Mar_ww_plm)
summary(plm.fit_Mar_ww)
