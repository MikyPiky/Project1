##########################################################################
#### Cross Validation to determine the model of WinterWheat for April ####
##########################################################################
...
formula_PrecTavgSmiGDM_2v3poly <- log(winterWheat) ~ poly(Prec_Apr, container_2v3poly[r, 1], raw = T) +  poly(Tavg_Apr, container_2v3poly[r, 2], raw = T) + 
...
for(r in 1:9){
  ...
  save.image(file = "/home/peichl/projects/crossvalid/cross4.RData")
}

## Load environment with data which has been produced on cluster
load("/home/peichl/Documents/projects/correlation/data/data_processed/Crossvalidation/winterWheat/cross4.RData")
...
BIC_loop_Apr_ww <- BIC_loop_2v8poly_Prec
cv.error_loocv_Apr_ww <- cv.error_loocv_2v8poly_Prec

par(mfrow=c(2,1))
plot(BIC_loop_2v8poly_Prec)
plot(cv.error_loocv_2v8poly_Prec)
plot(BIC_loop_Apr)
plot(cv.error_loocv_Apr, main="April", ylab="test MSE")

which.min(BIC_loop_Apr_ww)
which.min(cv.error_loocv_Apr_ww) 


# ich denke, dass hier Model Nummer 3am besten ist
r_Apr_ww = which.min(BIC_loop_Apr_ww)
r = r_Apr_ww
##########################
...
Yield_Covariates_nna_plm$SMI_Apr_within <- Within(Yield_Covariates_nna_plm$SMI_Apr)
Yield_Covariates_nna_plm$Prec_Apr_within <- Within(Yield_Covariates_nna_plm$Prec_Apr)
Yield_Covariates_nna_plm$Prec_Apr_within <- Within(Yield_Covariates_nna_plm$Tavg_Apr)
Yield_Covariates_nna_plm$SMI_Apr_GDM_within <- Within(Yield_Covariates_nna_plm$SMI_Apr_GDM)
sumApry(Yield_Covariates_nna_plm$Prec_Apr$)
...
Yield_Covariates_Apr <- Yield_Covariates_nna_plm[,Apr]
...
Yield_Covariates_Apr_ww <- cbind(Yield_Covariates_ww, Yield_Covariates_Apr)
names(Yield_Covariates_Apr_ww)
head(Yield_Covariates_Apr_ww)
names(Yield_Covariates_Apr_ww) <- c("year","comId","com","comIdState","comState","SHAPE_AREA","winterWheat", "SMI","Prec",
...
## Determine formula
r <- r_Apr_ww
container_2v3poly[r_Apr_ww, 1]
container_2v3poly[r_Apr_ww, 2]

formula_Apr_ww <- log(winterWheat) ~ poly(Prec, container_2v3poly[r, 1], raw = T) +  poly(Tavg, container_2v3poly[r, 2], raw = T) + dummy(SMI_GDM,c("severe drought","moderate drought","abnormal dry", "abnormal wet", "abundant wet","severe wet")) + log(as.integer(year)) + 
  dummy(comId)

####################
## PLM Ergebnisse ##
Yield_Covariates_Apr_ww_plm <- pdata.frame(Yield_Covariates_Apr_ww, index = c("comId","year"))
head(Yield_Covariates_Apr_ww_plm)
plm.fit_Apr_ww <- plm(formula= update(formula_Apr_ww, .~. - dummy(comId)),  data = Yield_Covariates_Apr_ww_plm)
summary(plm.fit_Apr_ww)
