...
## Output ##
- Yield_Covariates_WW_Aug_no2005.csv (auf August_no2005 reduzierter Data.Frame)
...
##### Delete year 2004 ####
###########################
table(Yield_Covariates_WW_Aug$year)
dim(Yield_Covariates_WW_Aug[Yield_Covariates_WW_Aug$year==2005,])
dim(Yield_Covariates_WW_Aug[Yield_Covariates_WW_Aug$year!=2005,])
dim(Yield_Covariates_WW_Aug)

Yield_Covariates_WW_Aug_no2005 <- Yield_Covariates_WW_Aug[Yield_Covariates_WW_Aug$year!=2005,]
dim(Yield_Covariates_WW_Aug_no2005)

head(Yield_Covariates_WW_Aug_no2005)

#########################################
#### Create stepwise function of SMI ####
#########################################
' Drought Monitor Spezification '
Yield_Covariates_WW_Aug_no2005$SMI_GDM <- cut(Yield_Covariates_WW_Aug_no2005$SMI, breaks = c(0, 0.1, 0.2, 0.3, 0.7, 0.8, 0.9, 1),                                            ,
                                              labels = c("severe drought","moderate drought","abnormal dry", "normal","abnormal wet" ,"abundant wet", "severe wet"))
#############
## Na-omit ##
sum(is.na(Yield_Covariates_WW_Aug_no2005) )
dim(Yield_Covariates_WW_Aug_no2005)
Yield_Covariates_WW_Aug_no2005_nna <- na.omit(Yield_Covariates_WW_Aug_no2005) 
dim(Yield_Covariates_WW_Aug_no2005_nna)

## Check for NAs
any(is.na(Yield_Covariates_WW_Aug_no2005_nna))
## Reset Rownames
rownames(Yield_Covariates_WW_Aug_no2005_nna) <- NULL

## Further work with DataFrame without Yield_Covariates_WW_Aug_no2005 index ##
Yield_Covariates_WW_Aug_no2005 <- Yield_Covariates_WW_Aug_no2005_nna
...
sum(table(Yield_Covariates_WW_Aug_no2005$comId) < 7 )
table(Yield_Covariates_WW_Aug_no2005$comId) < 7 
...
temp <- Yield_Covariates_WW_Aug_no2005
for (i in 1:length(list))
{
  print(Yield_Covariates_WW_Aug_no2005[Yield_Covariates_WW_Aug_no2005$comId==list[i],])
  temp <- (temp[!temp$comId==list[i],])
}

## Number of deleted rows
dim(temp)-dim(Yield_Covariates_WW_Aug_no2005)

## Further use old name for data.frame
Yield_Covariates_WW_Aug_no2005 <- temp

################################
## Befehle nach jedem löschen ##
Yield_Covariates_WW_Aug_no2005 <- na.omit(Yield_Covariates_WW_Aug_no2005) 
rownames(Yield_Covariates_WW_Aug_no2005) <- NULL
Yield_Covariates_WW_Aug_no2005 <- plm.data(Yield_Covariates_WW_Aug_no2005, index=c("comId", "year"))
Yield_Covariates_WW_Aug_no2005[,c("comId","stateId")] <- lapply(Yield_Covariates_WW_Aug_no2005[,c("comId","stateId")], factor )

#################################################
#### Remove log trend of indepedent variable ####
#################################################
'Fit log of yield on log of time and use the residuals of that for yields'
logtrend <- lm(log(Winterwheat) ~ log(as.integer(year)), data= Yield_Covariates_WW_Aug_no2005)
...
Yield_Covariates_WW_Aug_no2005[c(3009, 3154, 3165, 2358),]
'Hauptsächlich Orte im Osten und Bayern, Jahr 2003 zweimal'
...
logtrend <- lm(log(Winterwheat) ~ log(as.integer(year)), data= Yield_Covariates_WW_Aug_no2005)
summary(logtrend)
Yield_Covariates_WW_Aug_no2005$Winterwheat_logtrend <- resid(logtrend)

#######################################
## Prepare dataframe for plm package ##
'Change Indexing so that it can be used in plm package'
Yield_Covariates_WW_Aug_no2005 <- plm.data(Yield_Covariates_WW_Aug_no2005, index=c("comId", "year"))
str(Yield_Covariates_WW_Aug_no2005)


## Transform comId and stateId to factor ##
Yield_Covariates_WW_Aug_no2005[,c("comId","stateId")] <- lapply(Yield_Covariates_WW_Aug_no2005[,c("comId","stateId")], factor )
lapply(Yield_Covariates_WW_Aug_no2005, class)


###############################################
##### Save Yield_Covariates_WW_Aug_no2005ober extern ####
write.csv(Yield_Covariates_WW_Aug_no2005, file="./data/data_raw/Yield_Covariates_WW_Aug_no2005.csv")
...
for(r in 1:9){
  glm.fit_SMIPrecTavg  <- glm(formula = formula_Aug_WW_detrendlog_SMIPrecTavg, data = Yield_Covariates_WW_Aug_no2005) 
  BIC_SMIPrecTavg[r] <- BIC(glm.fit_SMIPrecTavg)
}

BIC_SMIPrecPet <- rep(0,9)
for(r in 1:9){
  glm.fit_SMIPrecPet  <- glm(formula = formula_Aug_WW_detrendlog_SMIPrecPet,  data = Yield_Covariates_WW_Aug_no2005)
  BIC_SMIPrecPet[r] <- BIC(glm.fit_SMIPrecPet)
}

BIC_SMIPrec <- rep(0,9)
for(r in 1:9){  
  glm.fit_SMIPrec      <- glm(formula = formula_Aug_WW_detrendlog_SMIPrec,     data = Yield_Covariates_WW_Aug_no2005) 
  BIC_SMIPrec[r] <- BIC(glm.fit_SMIPrec)
}

BIC_SMIPet <- rep(0,9)
for(r in 1:9){  
  glm.fit_SMIPet       <- glm(formula = formula_Aug_WW_detrendlog_SMIPet,      data = Yield_Covariates_WW_Aug_no2005) 
  BIC_SMIPet[r] <- BIC(glm.fit_SMIPet)
}

BIC_SMITavg <- rep(0,9)
for(r in 1:9){  
  glm.fit_SMITavg      <- glm(formula = formula_Aug_WW_detrendlog_SMITavg,     data = Yield_Covariates_WW_Aug_no2005) 
  BIC_SMITavg[r] <- BIC(glm.fit_SMITavg)
}

BIC_SMI <- rep(0,9)
for(r in 1:9){  
  glm.fit_SMI      <- glm(formula = formula_Aug_WW_detrendlog_SMI,     data = Yield_Covariates_WW_Aug_no2005) 
  BIC_SMI[r] <- BIC(glm.fit_SMI)
}

BIC_PrecTavg <- rep(0,9)
for(r in 1:9){
  glm.fit_PrecTavg  <- glm(formula = formula_Aug_WW_detrendlog_PrecTavg, data = Yield_Covariates_WW_Aug_no2005) 
  BIC_PrecTavg[r] <- BIC(glm.fit_PrecTavg)
}

BIC_PrecPet <- rep(0,9)
for(r in 1:9){
  glm.fit_PrecPet  <- glm(formula = formula_Aug_WW_detrendlog_PrecPet, data = Yield_Covariates_WW_Aug_no2005) 
  BIC_PrecPet[r] <- BIC(glm.fit_PrecPet)
}

BIC_Prec <- rep(0,9)
for(r in 1:9){
  glm.fit_Prec  <- glm(formula = formula_Aug_WW_detrendlog_Prec, data = Yield_Covariates_WW_Aug_no2005) 
  BIC_Prec[r] <- BIC(glm.fit_Prec)
}

BIC_Pet  <- rep(0,9)
for(r in 1:9){
  glm.fit_Pet   <- glm(formula = formula_Aug_WW_detrendlog_Pet , data = Yield_Covariates_WW_Aug_no2005) 
  BIC_Pet [r] <- BIC(glm.fit_Pet )
}

BIC_Tavg  <- rep(0,9)
for(r in 1:9){
  glm.fit_Tavg   <- glm(formula = formula_Aug_WW_detrendlog_Tavg , data = Yield_Covariates_WW_Aug_no2005) 
...
index <- 1:99
month <-rep("August_no2005",99)

BIC_Aug_no2005 <- cbind(BIC, model ,model_index, index, month)

#######################
## Delete Duplicates ##
which(duplicated(BIC_Aug_no2005$BIC))
...
for (i in 1:44)
{
  print(BIC_Aug_no2005[BIC_Aug_no2005$index ==list3[i],])
...
rownames(temp) <- NULL
BIC_Aug_no2005 <- temp
lapply(BIC_Aug_no2005, class)

############################
## Plot data with ggplot2 ##
g <- ggplot(BIC_Aug_no2005,aes(y=BIC, x=index))
...
g + geom_point(aes(color=model)) + labs(title="BIC of various model configurations", x="") + theme(plot.title=element_text(size=15, face="bold")) + theme_dark() +
  facet_wrap( ~ month)
sort(BIC_Aug_no2005$BIC[1:9])
which.min(BIC_Aug_no2005$BIC[1:9])
## Export Data frame for use in BIC_Grafic
BIC_WW_Aug_no2005 <- BIC_Aug_no2005
class(BIC_WW_Aug_no2005)
write.csv(BIC_WW_Aug_no2005, file="./data/data_raw/BIC/BIC_WW_Aug_no2005.csv")
...
###################
## Load Data Set ##
# Yield_Covariates_WW_Aug_no2005 <- read.csv( file="./data/data_raw/Yield_Covariates_WW_Aug_no2005.csv")
# names(Yield_Covariates_WW_Aug_no2005)
# Yield_Covariates_WW_Aug_no2005$X <- NULL
...
table(Yield_Covariates_WW_Aug_no2005$year)
#################################
###############################
...
r = 3 
' Aufgrund von Singularität nehme ich drei, da dies am nächst Größten ist.'
best_formula <- formula_Aug_WW_detrendlog_SMIPrecTavg

###################
## GLM Ergebniss ##
glm.fit_WW_BEST_Aug_no2005  <- glm(formula = best_formula,  data = Yield_Covariates_WW_Aug_no2005)
summary(glm.fit_WW_BEST_Aug_no2005)
'AIC: -6918.4'

####################
## PLM Ergebnisse ##
plm.fit_WW_BEST_Aug_no2005 <- plm(formula = update(best_formula, .~. - dummy(comId)),  data = Yield_Covariates_WW_Aug_no2005,  effect="individual", model=("within"), index = c("comId","year"))
summary(plm.fit_WW_BEST_Aug_no2005)
'Adj. R-Squared: 0.19607'

fixef <-  fixef(plm.fit_WW_BEST_Aug_no2005)
...
fixef
write.csv(fixef, "./figures/figures_exploratory/FixedEffects/Winterwheat/plm.fit_WW_BEST_Aug_FE_no2005.csv")

##################
## LM Ergebniss ##
lm.fit_WW_BEST_Aug_no2005  <-lm(formula = best_formula,  data = Yield_Covariates_WW_Aug_no2005)
summary(lm.fit_WW_BEST_Aug_no2005) 
'  Adjusted R-squared:0.7012'

########################
## Heteroskedasdicity ##
bptest(glm.fit_WW_BEST_Aug_no2005) # Breusch Pagan Test of Heteroskedastie in den Störgrößen: Null: Homoskedasdicity.
bptest(plm.fit_WW_BEST_Aug_no2005)
...
bptest(plm.fit_WW_BEST_Aug_no2005, studentize = TRUE)
...
pwartest(plm.fit_WW_BEST_Aug_no2005)
pbgtest(plm.fit_WW_BEST_Aug_no2005) 
'
...
coeftest(plm.fit_WW_BEST_Aug_no2005)

## Robust covariance matrix estimators a la White ##
# coeftest(plm.fit_WW_BEST_Aug_no2005,vcov=vcovHC(plm.fit_WW_BEST_Aug_no2005,method = "arellano", type = "HC0")) 
cov0_WW_BEST_Aug_no2005 <- vcovHC(plm.fit_WW_BEST_Aug_no2005,method = "arellano", type = "HC0", cluster="group")
Wh.se_serial_WW_BEST_Aug_no2005 <- sqrt(diag(cov0_WW_BEST_Aug_no2005))

cov0.1_WW_BEST_Aug_no2005 <- vcovHC(plm.fit_WW_BEST_Aug_no2005,method = "arellano", type = "HC0", cluster="time")
Wh.se_cross_WW_BEST_Aug_no2005 <- sqrt(diag(cov0.1_WW_BEST_Aug_no2005))
# 
# ## Beck Katz:
# # coeftest(plm.fit_WW_BEST_Aug_no2005, vcov = function(x) vcovBK(plm.fit_WW_BEST_Aug_no2005,method = "arellano", type = "HC0"))
# cov1 <- vcovBK(plm.fit_WW_BEST_Aug_no2005,method = "arellano", type = "HC0",  cluster="time")
# BK.se <- sqrt(diag(cov1))

# ## Driscoll Kraay ##
# summary(plm.fit_WW_BEST_Aug_no2005)
coeftest(plm.fit_WW_BEST_Aug_no2005,  vcov=function(x) vcovSCC(plm.fit_WW_BEST_Aug_no2005,method = "arellano",type = "HC0")) 
cov2_WW_BEST_Aug_no2005     <- vcovSCC(plm.fit_WW_BEST_Aug_no2005,method = "arellano",type = "HC0")
DK.se_WW_BEST_Aug_no2005    <- sqrt(diag(cov2_WW_BEST_Aug_no2005))
# 
# cov2.1_WW_BEST_Aug_no2005   <- vcovSCC(plm.fit_WW_BEST_Aug_no2005,method = "arellano",type = "HC0", maxlag=1)
# DK2.1.se_WW_BEST_Aug_no2005 <- sqrt(diag(cov2.1_WW_BEST_Aug_no2005))

# cov2.2_WW_BEST_Aug_no2005   <- vcovSCC(plm.fit_WW_BEST_Aug_no2005,method = "arellano",type = "HC0", maxlag=2)
# DK2.2.se_WW_BEST_Aug_no2005 <- sqrt(diag(cov2.2_WW_BEST_Aug_no2005))
# 
# cov2.3_WW_BEST_Aug_no2005   <- vcovSCC(plm.fit_WW_BEST_Aug_no2005,method = "arellano",type = "HC0", maxlag=3)
# DK2.3.se_WW_BEST_Aug_no2005 <- sqrt(diag(cov2.3_WW_BEST_Aug_no2005))
# 
# cov2.4_WW_BEST_Aug_no2005   <- vcovSCC(plm.fit_WW_BEST_Aug_no2005,method = "arellano",type = "HC0", maxlag=4)
# DK2.4.se_WW_BEST_Aug_no2005 <- sqrt(diag(cov2.4_WW_BEST_Aug_no2005))
# 
cov2.5_WW_BEST_Aug_no2005   <- vcovSCC(plm.fit_WW_BEST_Aug_no2005,method = "arellano",type = "HC0", maxlag=5)
DK2.5.se_WW_BEST_Aug_no2005 <- sqrt(diag(cov2.5_WW_BEST_Aug_no2005))

## Cameron et al /Thompson : doouble-clustering estimator  ##
# coeftest(plm.fit_WW_BEST_Aug_no2005, vcovDC(plm.fit_WW_BEST_Aug_no2005, method = "arellano", type = "HC0"))
cov3_WW_BEST_Aug_no2005 <- vcovDC(plm.fit_WW_BEST_Aug_no2005, method = "arellano", type = "HC0")
CT.se_WW_BEST_Aug_no2005 <- sqrt(diag(cov3_WW_BEST_Aug_no2005))
...
se <- list(NULL, Wh.se_cross_WW_BEST_Aug_no2005, Wh.se_serial_WW_BEST_Aug_no2005,  DK.se_WW_BEST_Aug_no2005, DK2.5.se_WW_BEST_Aug_no2005, CT.se_WW_BEST_Aug_no2005)
labels1 <-c("NULL","WhiteCross","WhiteSerial", "DriscollKraay", "DriscollKray2.5","CameronThompson")
stargazer(plm.fit_WW_BEST_Aug_no2005, plm.fit_WW_BEST_Aug_no2005, plm.fit_WW_BEST_Aug_no2005, plm.fit_WW_BEST_Aug_no2005, plm.fit_WW_BEST_Aug_no2005,plm.fit_WW_BEST_Aug_no2005,
          se = se,   
          dep.var.caption  = "Model with smallest BIC - August_no2005",
          dep.var.labels = "log(Winterwheat)",
          style="default",
          model.numbers = FALSE,
          column.labels = labels1,          
          type="text", out="./figures/figures_exploratory/BIC/Winterwheat/WW_Aug_no2005_best.txt"
)
...
#########################################################
' Same as best BIC general model'
...
stargazer(plm.fit_WW_BEST_Aug_no2005, plm.fit_WW_BEST_Aug_no2005, plm.fit_WW_BEST_Aug_no2005, plm.fit_WW_BEST_Aug_no2005, plm.fit_WW_BEST_Aug_no2005,plm.fit_WW_BEST_Aug_no2005,
          ...
          model.numbers = FALSE,
          column.labels = labels1,           
          type="text", out="./figures/figures_exploratory/BIC/Winterwheat/WW_Aug_no2005_bestStandard.txt"
)
...
stargazer(plm.fit_WW_BEST_Aug_no2005, plm.fit_WW_BEST_Aug_no2005, plm.fit_WW_BEST_Aug_no2005, plm.fit_WW_BEST_Aug_no2005, plm.fit_WW_BEST_Aug_no2005,plm.fit_WW_BEST_Aug_no2005,
          ...
          model.numbers = FALSE,
          column.labels = labels1,                          
          type="text", out="./figures/figures_exploratory/BIC/Winterwheat/WW_Aug_no2005_bestSM.txt"
)
