...
## Output ##
- Yield_Covariates_WW_Nov_lag.csv (auf November_lag reduzierter Data.Frame)
- Export Data frame for use in BIC_Graphic: file="./data/data_raw/BIC/BIC_WW_Nov_lag.csv")
...
###############################################
##### Save Yield_Covariates_WW_Nov_lagober extern ####
write.csv(Yield_Covariates_WW_Nov_lag, file="./data/data_raw/Yield_Covariates_WW_Nov_lag.csv")
...
BIC_Nov_lag <- cbind(BIC, model ,model_index, index, month)

#######################
## Delete Duplicates ##
which(duplicated(BIC_Nov_lag$BIC))
...
for (i in 1:44)
{
  print(BIC_Nov_lag[BIC_Nov_lag$index ==list3[i],])
...
rownames(temp) <- NULL
BIC_Nov_lag <- temp
lapply(BIC_Nov_lag, class)

############################
## Plot data with ggplot2 ##
g <- ggplot(BIC_Nov_lag,aes(y=BIC, x=index))
...
BIC_WW_Nov_lag <- BIC_Nov_lag
class(BIC_WW_Nov_lag)
write.csv(BIC_WW_Nov_lag, file="./data/data_raw/BIC/BIC_WW_Nov_lag.csv")
...
###################
## Load Data Set ##
# Yield_Covariates_WW_Nov_lag <- read.csv( file="./data/data_raw/Yield_Covariates_WW_Nov_lag.csv")
# names(Yield_Covariates_WW_Nov_lag)
# Yield_Covariates_WW_Nov_lag$X <- NULL
...
r = 6
best_formula <- formula_Nov_lag_WW_detrendlog_SMIPrecPet


###################
## GLM Ergebniss ##
glm.fit_WW_BEST_Nov_lag  <- glm(formula = best_formula,  data = Yield_Covariates_WW_Nov_lag)
summary(glm.fit_WW_BEST_Nov_lag)
'AIC:  -8006.2'

####################
## PLM Ergebnisse ##
plm.fit_WW_BEST_Nov_lag <- plm(formula = update(best_formula, .~. - dummy(comId)),  data = Yield_Covariates_WW_Nov_lag,  effect="individual", model=("within"), index = c("comId","year"))
summary(plm.fit_WW_BEST_Nov_lag)
'Adj. R-Squared: 0.22546'

fixef <-  fixef(plm.fit_WW_BEST_Nov_lag)
...
fixef
write.csv(fixef, "./figures/figures_exploratory/FixedEffects/Winterwheat/plm.fit_WW_BEST_Nov_lag_FE.csv")

##################
## LM Ergebniss ##
lm.fit_WW_BEST_Nov_lag  <-lm(formula = best_formula,  data = Yield_Covariates_WW_Nov_lag)
summary(lm.fit_WW_BEST_Nov_lag) 
'Adjusted R-squared:   0.7251 '

################################################
## Assessing Influence (Leverage*discrepancy) ##
cutoff_Nov_lag <- 4/((nrow(Yield_Covariates_WW_Nov_lag)-length(lm.fit_WW_BEST_Nov_lag$coefficients)-1)) 
cutoff_Nov_lag
plot(lm.fit_WW_BEST_Nov_lag, which=4, cook_Nov_lag.levels=cutoff_Nov_lag)
cook_Nov_lag <- cooks.distance(lm.fit_WW_BEST_Nov_lag)

nrow(Yield_Covariates_WW_Nov_lag[cook_Nov_lag > cutoff_Nov_lag,]) # 195

year_cooks_Nov_lag <- table(Yield_Covariates_WW_Nov_lag$year[cook_Nov_lag > cutoff_Nov_lag ]) 
year_cooks_Nov_lag
'1999 2000 2001 2002 2003 2004 2005 2006 2007 2008 2009 2010 
   7   14   10    9   43   23   10   10   21   24   11   13 
'

com_cooks_Nov_lag <- sort(table(Yield_Covariates_WW_Nov_lag$com[cook_Nov_lag > cutoff_Nov_lag ] ) )
tail(com_cooks_Nov_lag,20)
'
                       Weimar, krsfr. Stadt       Bad T\xf6lz-Wolfratshausen, Landkreis                           Barnim, Landkreis             Berchtesgadener Land, Landkreis 
                                          2                                           3                                           3                                           3 
                      G\xf6rlitz, Landkreis                             Lahn-Dill-Kreis                 Magdeburg, Kreisfreie Stadt                      Nordsachsen, Landkreis 
                                          3                                           3                                           3                                           3 
                 Rheinisch-Bergischer Kreis                Salzgitter, Kreisfreie Stadt                   Spree-Nei\xdfe, Landkreis                  Wolfenb\xfcttel, Landkreis 
                                          3                                           3                                           3                                           3 
                       Havelland, Landkreis                     Landkreis Uecker-Randow                              Leipzig, Stadt        M\xf6nchengladbach, Kreisfreie Stadt 
                                          4                                           4                                           4                                           4 
                       Oberhavel, Landkreis                                 Olpe, Kreis               Potsdam-Mittelmark, Landkreis            Oberspreewald-Lausitz, Landkreis 
                                          4                                           4                                           4                                           5 '


########################
## Heteroskedasdicity ##
bptest(glm.fit_WW_BEST_Nov_lag) # Breusch Pagan Test of Heteroskedastie in den Störgrößen: Null: Homoskedasdicity.
bptest(plm.fit_WW_BEST_Nov_lag)
...
bptest(plm.fit_WW_BEST_Nov_lag, studentize = TRUE)
...
pwartest(plm.fit_WW_BEST_Nov_lag)
pbgtest(plm.fit_WW_BEST_Nov_lag) 
'
...
coeftest(plm.fit_WW_BEST_Nov_lag)

## Robust covariance matrix estimators a la White ##
# coeftest(plm.fit_WW_BEST_Nov_lag,vcov=vcovHC(plm.fit_WW_BEST_Nov_lag,method = "arellano", type = "HC0")) 
cov0_WW_BEST_Nov_lag <- vcovHC(plm.fit_WW_BEST_Nov_lag,method = "arellano", type = "HC0", cluster="group")
Wh.se_serial_WW_BEST_Nov_lag <- sqrt(diag(cov0_WW_BEST_Nov_lag))

cov0.1_WW_BEST_Nov_lag <- vcovHC(plm.fit_WW_BEST_Nov_lag,method = "arellano", type = "HC0", cluster="time")
Wh.se_cross_WW_BEST_Nov_lag <- sqrt(diag(cov0.1_WW_BEST_Nov_lag))
# 
# ## Beck Katz:
# # coeftest(plm.fit_WW_BEST_Nov_lag, vcov = function(x) vcovBK(plm.fit_WW_BEST_Nov_lag,method = "arellano", type = "HC0"))
# cov1 <- vcovBK(plm.fit_WW_BEST_Nov_lag,method = "arellano", type = "HC0",  cluster="time")
# BK.se <- sqrt(diag(cov1))

# ## Driscoll Kraay ##
# summary(plm.fit_WW_BEST_Nov_lag)
coeftest(plm.fit_WW_BEST_Nov_lag,  vcov=function(x) vcovSCC(plm.fit_WW_BEST_Nov_lag,method = "arellano",type = "HC0")) 
cov2_WW_BEST_Nov_lag     <- vcovSCC(plm.fit_WW_BEST_Nov_lag,method = "arellano",type = "HC0")
DK.se_WW_BEST_Nov_lag    <- sqrt(diag(cov2_WW_BEST_Nov_lag))
# 
# cov2.1_WW_BEST_Nov_lag   <- vcovSCC(plm.fit_WW_BEST_Nov_lag,method = "arellano",type = "HC0", maxlag=1)
# DK2.1.se_WW_BEST_Nov_lag <- sqrt(diag(cov2.1_WW_BEST_Nov_lag))

# cov2.2_WW_BEST_Nov_lag   <- vcovSCC(plm.fit_WW_BEST_Nov_lag,method = "arellano",type = "HC0", maxlag=2)
# DK2.2.se_WW_BEST_Nov_lag <- sqrt(diag(cov2.2_WW_BEST_Nov_lag))
# 
# cov2.3_WW_BEST_Nov_lag   <- vcovSCC(plm.fit_WW_BEST_Nov_lag,method = "arellano",type = "HC0", maxlag=3)
# DK2.3.se_WW_BEST_Nov_lag <- sqrt(diag(cov2.3_WW_BEST_Nov_lag))
# 
# cov2.4_WW_BEST_Nov_lag   <- vcovSCC(plm.fit_WW_BEST_Nov_lag,method = "arellano",type = "HC0", maxlag=4)
# DK2.4.se_WW_BEST_Nov_lag <- sqrt(diag(cov2.4_WW_BEST_Nov_lag))
# 
cov2.5_WW_BEST_Nov_lag   <- vcovSCC(plm.fit_WW_BEST_Nov_lag,method = "arellano",type = "HC0", maxlag=5)
DK2.5.se_WW_BEST_Nov_lag <- sqrt(diag(cov2.5_WW_BEST_Nov_lag))

## Cameron et al /Thompson : doouble-clustering estimator  ##
# coeftest(plm.fit_WW_BEST_Nov_lag, vcovDC(plm.fit_WW_BEST_Nov_lag, method = "arellano", type = "HC0"))
cov3_WW_BEST_Nov_lag <- vcovDC(plm.fit_WW_BEST_Nov_lag, method = "arellano", type = "HC0")
CT.se_WW_BEST_Nov_lag <- sqrt(diag(cov3_WW_BEST_Nov_lag))
...
se <- list(NULL, Wh.se_cross_WW_BEST_Nov_lag, Wh.se_serial_WW_BEST_Nov_lag,  DK.se_WW_BEST_Nov_lag, DK2.5.se_WW_BEST_Nov_lag, CT.se_WW_BEST_Nov_lag)
labels1 <-c("NULL","WhiteCross","WhiteSerial", "DriscollKraay", "DriscollKray2.5","CameronThompson")
stargazer(plm.fit_WW_BEST_Nov_lag, plm.fit_WW_BEST_Nov_lag, plm.fit_WW_BEST_Nov_lag, plm.fit_WW_BEST_Nov_lag, plm.fit_WW_BEST_Nov_lag,plm.fit_WW_BEST_Nov_lag,
          se = se,   
          ...
          type="text", out="./figures/figures_exploratory/BIC/Winterwheat/WW_Nov_lag_best.txt"
)
...
r = 6
bestStandard_formula <- formula_Nov_lag_WW_detrendlog_SMIPrecTavg


###################
## GLM Ergebniss ##
glm.fit_WW_bestStandard_Nov_lag  <- glm(formula = bestStandard_formula,  data = Yield_Covariates_WW_Nov_lag)
summary(glm.fit_WW_bestStandard_Nov_lag)
'AIC: -7842.3'

####################
## PLM Ergebnisse ##
plm.fit_WW_bestStandard_Nov_lag <- plm(formula = update(bestStandard_formula, .~. - dummy(comId)),  data = Yield_Covariates_WW_Nov_lag,  effect="individual", model=("within"), index = c("comId","year"))
summary(plm.fit_WW_bestStandard_Nov_lag)
'Adj. R-Squared: 0.19761'

## Generate Fixed Effects data.frame and export it ##
fixef <- fixef(plm.fit_WW_bestStandard_Nov_lag)
fixef <- as.data.frame(as.matrix(fixef))
head(fixef)
fixef <- cbind(rownames(fixef), fixef)
rownames(fixef) <- NULL
names(fixef) <- c("comId", "FE")
write.csv(fixef, "./figures/figures_exploratory/FixedEffects/Winterwheat/plm.fit_WW_bestStandard_Nov_lag_FE.csv")

##################
## LM Ergebniss ##
lm.fit_WW_bestStandard_Nov_lag  <-lm(formula = bestStandard_formula,  data = Yield_Covariates_WW_Nov_lag)
summary(lm.fit_WW_bestStandard_Nov_lag) 
'Adjusted R-squared:  0.714 '

########################
## Heteroskedasdicity ##
bptest(glm.fit_WW_bestStandard_Nov_lag) # Breusch Pagan Test of Heteroskedastie in den Störgrößen: Null: Homoskedasdicity.
bptest(plm.fit_WW_bestStandard_Nov_lag)
' In beiden Fällen kann die Null widerlegt werden. Es gibt also heteroskedasdicity '

## Koenkers Version on BP Test: robuste Modification wenn die Störgrößen nicht normalverteilt sind. 
bptest(plm.fit_WW_bestStandard_Nov_lag, studentize = TRUE)
'Auch hier kann die Null widerlegt werden. Need to use robust covariance variance matrix to correct standard errors'
...
######################################
## Tests for serial autocorrelation ##
pwartest(plm.fit_WW_bestStandard_Nov_lag)
' Hier serielle Korrelation festzustellen'
pbgtest(plm.fit_WW_bestStandard_Nov_lag) 
...
## Correct Standard Errors used in table ##
coeftest(plm.fit_WW_bestStandard_Nov_lag)

## Robust covariance matrix estimators a la White 
# coeftest(plm.fit_WW_bestStandard_Nov_lag,vcov=vcovHC(plm.fit_WW_bestStandard_Nov_lag,method = "arellano", type = "HC0")) 
cov0_WW_bestStandard_Nov_lag <- vcovHC(plm.fit_WW_bestStandard_Nov_lag,method = "arellano", type = "HC0", cluster="group")
Wh.se_serial_WW_bestStandard_Nov_lag <- sqrt(diag(cov0_WW_bestStandard_Nov_lag))

cov0.1_WW_bestStandard_Nov_lag <- vcovHC(plm.fit_WW_bestStandard_Nov_lag,method = "arellano", type = "HC0", cluster="time")
Wh.se_cross_WW_bestStandard_Nov_lag <- sqrt(diag(cov0.1_WW_bestStandard_Nov_lag))

# ## Beck Katz ##
# # coeftest(plm.fit_WW_bestStandard_Nov_lag, vcov = function(x) vcovBK(plm.fit_WW_bestStandard_Nov_lag,method = "arellano", type = "HC0"))
# cov1_WW_bestStandard_Nov_lag <- vcovBK(plm.fit_WW_bestStandard_Nov_lag,method = "arellano", type = "HC0",  cluster="time")
# BK.se_WW_bestStandard_Nov_lag <- sqrt(diag(cov1_WW_bestStandard_Nov_lag))

## Driscoll Kraay: ##
summary(plm.fit_WW_bestStandard_Nov_lag)
cov2_WW_bestStandard_Nov_lag     <- vcovSCC(plm.fit_WW_bestStandard_Nov_lag,method = "arellano",type = "HC0")
DK.se_WW_bestStandard_Nov_lag    <- sqrt(diag(cov2_WW_bestStandard_Nov_lag))

# cov2.1_WW_bestStandard_Nov_lag   <- vcovSCC(plm.fit_WW_bestStandard_Nov_lag,method = "arellano",type = "HC0", maxlag=1)
# DK2.1.se_WW_bestStandard_Nov_lag <- sqrt(diag(cov2.1_WW_bestStandard_Nov_lag))

# cov2.2_WW_bestStandard_Nov_lag   <- vcovSCC(plm.fit_WW_bestStandard_Nov_lag,method = "arellano",type = "HC0", maxlag=2)
# DK2.2.se_WW_bestStandard_Nov_lag <- sqrt(diag(cov2.2_WW_bestStandard_Nov_lag))
# 
# cov2.3_WW_bestStandard_Nov_lag   <- vcovSCC(plm.fit_WW_bestStandard_Nov_lag,method = "arellano",type = "HC0", maxlag=3)
# DK2.3.se_WW_bestStandard_Nov_lag <- sqrt(diag(cov2.3_WW_bestStandard_Nov_lag))
# 
# cov2.4_WW_bestStandard_Nov_lag   <- vcovSCC(plm.fit_WW_bestStandard_Nov_lag,method = "arellano",type = "HC0", maxlag=4)
# DK2.4.se_WW_bestStandard_Nov_lag <- sqrt(diag(cov2.4_WW_bestStandard_Nov_lag))
# 
cov2.5_WW_bestStandard_Nov_lag   <- vcovSCC(plm.fit_WW_bestStandard_Nov_lag,method = "arellano",type = "HC0", maxlag=5)
DK2.5.se_WW_bestStandard_Nov_lag <- sqrt(diag(cov2.5_WW_bestStandard_Nov_lag))

## Cameron et al /Thompson : doouble-clustering estimator 
# coeftest(plm.fit_WW_bestStandard_Nov_lag, vcovDC(plm.fit_WW_bestStandard_Nov_lag, method = "arellano", type = "HC0"))
cov3_WW_bestStandard_Nov_lag <- vcovDC(plm.fit_WW_bestStandard_Nov_lag, method = "arellano", type = "HC0")
CT.se_WW_bestStandard_Nov_lag <- sqrt(diag(cov3_WW_bestStandard_Nov_lag))
'Our estimator is qualitatively similar to the ones presented in White and Domowitz (1984), for
time series data, and Conley (1999), for spatial data. '

## Generate Table with Output ##
se <- list(NULL, Wh.se_cross_WW_bestStandard_Nov_lag, Wh.se_serial_WW_bestStandard_Nov_lag, DK.se_WW_bestStandard_Nov_lag, DK2.5.se_WW_bestStandard_Nov_lag, CT.se_WW_bestStandard_Nov_lag)
labels1 <-c("NULL","WhiteCross","WhiteSerial", "DriscollKraay", "DriscollKray2.5","CameronThompson")
stargazer(plm.fit_WW_bestStandard_Nov_lag, plm.fit_WW_bestStandard_Nov_lag, plm.fit_WW_bestStandard_Nov_lag, plm.fit_WW_bestStandard_Nov_lag, plm.fit_WW_bestStandard_Nov_lag,plm.fit_WW_bestStandard_Nov_lag,
          se = se,   
          ...
          model.numbers = FALSE,
          column.labels = labels1,          
          type="text", out="./figures/figures_exploratory/BIC/Winterwheat/WW_Nov_lag_bestStandard.txt"
)
...
stargazer(plm.fit_WW_BEST_Nov_lag, plm.fit_WW_BEST_Nov_lag, plm.fit_WW_BEST_Nov_lag, plm.fit_WW_BEST_Nov_lag, plm.fit_WW_BEST_Nov_lag,plm.fit_WW_BEST_Nov_lag,
          ...
          model.numbers = FALSE,
          column.labels = labels1, 
          type="text", out="./figures/figures_exploratory/BIC/Winterwheat/WW_Nov_lag_bestSM.txt"
)
