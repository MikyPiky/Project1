...
names_Mar_no2004 <- grep(c("*_Mar"), names)
names_Mar
...
g + geom_point(aes(color=model)) + labs(title="BIC of various model configurations", x="") + theme(plot.title=element_text(size=15, face="bold")) + theme_dark() +
  facet_wrap( ~ month)
BIC_Mar
which.min(BIC_Mar$BIC)

BIC_Mar[which.min(BIC_Mar$BIC),]


## Export Data frame for use in BIC_Grafic
BIC_WW_Mar <- BIC_Mar
class(BIC_WW_Mar)
write.csv(BIC_WW_Mar, file="./data/data_raw/BIC/BIC_WW_Mar_no2004.csv")
...
###################
## GLM Ergebniss ##
glm.fit_WW_BEST_Mar  <- glm(formula = best_formula,  data = Yield_Covariates_WW_Mar_no2004)
summary(glm.fit_WW_BEST_Mar)
'AIC: -7466.3 '

####################
## PLM Ergebnisse ##
plm.fit_WW_BEST_Mar <- plm(formula = update(best_formula, .~. - dummy(comId)),  data = Yield_Covariates_WW_Mar_no2004,  effect="individual", model=("within"), index = c("comId","year"))
...
##################
## LM Ergebniss ##
lm.fit_WW_BEST_Mar  <-lm(formula = best_formula,  data = Yield_Covariates_WW_Mar_no2004)
