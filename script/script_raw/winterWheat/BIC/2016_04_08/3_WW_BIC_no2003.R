...
BIC_WW_Mar <- BIC_Mar
class(BIC_WW_Mar)
write.csv(BIC_WW_Mar, file="./data/data_raw/BIC/BIC_WW_Mar_no2003.csv")
...
###################
## GLM Ergebniss ##
glm.fit_WW_BEST_Mar  <- glm(formula = best_formula,  data = Yield_Covariates_WW_Mar_no2003)
summary(glm.fit_WW_BEST_Mar)
'AIC: -7906.5'

####################
## PLM Ergebnisse ##
plm.fit_WW_BEST_Mar <- plm(formula = update(best_formula, .~. - dummy(comId)),  data = Yield_Covariates_WW_Mar_no2003,  effect="individual", model=("within"), index = c("comId","year"))
...
##################
## LM Ergebniss ##
lm.fit_WW_BEST_Mar  <-lm(formula = best_formula,  data = Yield_Covariates_WW_Mar_no2003)
