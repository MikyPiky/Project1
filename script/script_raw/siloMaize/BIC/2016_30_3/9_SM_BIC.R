...
head(Yield_Covariates_SM_Sep_nna)
...
# ## Leverage one ##
# # Warnings in plot()
# Yield_Covariates_SM_Sep[c(181, 182, 765, 851, 1006, 1557) ,]
# 
# # Crucial Observations in plot()
# Yield_Covariates_SM_Sep[c(3335,3673,3219, 3681, 1557),]

## Anschauem der Struktur der coms, welche hier ausgegeben wurden: hier sind vor allem die SiloMaize Werte relevant ##
Yield_Covariates_SM_Sep[Yield_Covariates_SM_Sep$comId == "3101",] # < 1 Beobachtung
Yield_Covariates_SM_Sep[Yield_Covariates_SM_Sep$comId == "3102",] # # < 1 Beobachtung
Yield_Covariates_SM_Sep[Yield_Covariates_SM_Sep$comId == "5314",] # < 1 Beobachtung 
Yield_Covariates_SM_Sep[Yield_Covariates_SM_Sep$comId == "5512",] # < 1 Beobachtung
Yield_Covariates_SM_Sep[Yield_Covariates_SM_Sep$comId == "5911",] # < 1 Beobachtung
Yield_Covariates_SM_Sep[Yield_Covariates_SM_Sep$comId == "7331",] # < 1 Beobachtung

Yield_Covariates_SM_Sep[Yield_Covariates_SM_Sep$comId == "15001",] # < 4 Beobachtungen
Yield_Covariates_SM_Sep[Yield_Covariates_SM_Sep$comId == "12069",] # hier mache ich am besten nichts
Yield_Covariates_SM_Sep[Yield_Covariates_SM_Sep$comId == "14730",] # hier mache ich auch am besten nichts
Yield_Covariates_SM_Sep[Yield_Covariates_SM_Sep$comId == "12053",] # hier mache ich auch am besten nichts
...
attach(Yield_Covariates_SM_Sep)
...
Yield_Covariates_SM_Sep[Yield_Covariates_SM_Sep$comId == "12067",] # 2006 hier scheint nur ein Jahr ein Messfehler zu sein: diesen Lösche ich 
Yield_Covariates_SM_Sep[Yield_Covariates_SM_Sep$comId == "12069",] # 2003 hier scheint nur ein Jahr ein Messfehler zu sein: diesen Lösche ich 
Yield_Covariates_SM_Sep[Yield_Covariates_SM_Sep$comId == "12067",] # 2006 hier scheint nur ein Jahr ein Messfehler zu sein: diesen Lösche ich 

Yield_Covariates_SM_Sep[Yield_Covariates_SM_Sep$comId == "12060",] # verändere ich nicht 


## Delete outliers ##
Yield_Covariates_SM_Sep <- Yield_Covariates_SM_Sep[!(Yield_Covariates_SM_Sep$comId == "6532" & Yield_Covariates_SM_Sep$year == "2008"),]
Yield_Covariates_SM_Sep <- Yield_Covariates_SM_Sep[!(Yield_Covariates_SM_Sep$comId == "12067" & Yield_Covariates_SM_Sep$year == "2006"),]
Yield_Covariates_SM_Sep <- Yield_Covariates_SM_Sep[!(Yield_Covariates_SM_Sep$comId == "12069" & Yield_Covariates_SM_Sep$year == "2003"),]
Yield_Covariates_SM_Sep <- Yield_Covariates_SM_Sep[!(Yield_Covariates_SM_Sep$comId == "12067" & Yield_Covariates_SM_Sep$year == "2006"),]
...
formula_Sep_sm_detrendlog_SMIPet <- siloMaize_logtrend ~ poly(Pet, degree[r, 1], raw = T) +  
  dummy(SMI_GDM,c("severe drought","moderate drought","abnormal dry", "abnormal wet", "abundant wet","severe wet")) + dummy(comId)
...
formula_Sep_sm_detrendlog_Pet <- siloMaize_logtrend ~ poly(Pet, degree[r, 1], raw = T)  + dummy(comId)
...
#   cv.error[r] <- cv.glm( Yield_Covariates_SM_Sep, glm.fit )$delta[1] # hat funktioniert (dummies dürfen nicht nur als Faktor definiert werden)
...
# cv.error_loocv_Sep_SiloMaize <- cv.error
...
BIC_SM_9 <- BIC2
class(BIC_SM_9)
write.csv(BIC_SM_9, file="./data/data_raw/BIC/BIC_SM_9.csv")
...
# dim( Yield_Covariates_SM_Sep)
# head(Yield_Covariates_SM_Sep)
# summary(Yield_Covariates_SM_Sep$siloMaize)
# hist(Yield_Covariates_SM_Sep$siloMaize)
# 
# 
...
# na.omit(Yield_Covariates_SM_Sep[rstd > 5 | rstd < -5,])  #11)
# 
# ## Generate data without outliers rstd > |6.5|
# na.omit(Yield_Covariates_SM_Sep[rstd > 6 | rstd < -6,])  
# dim(Yield_Covariates_SM_Sep)
# temp <-  Yield_Covariates_SM_Sep[!rstd > 6 & !rstd < -6,]
# dim(temp)
# 
...
r=5
glm.fit3 <- glm(formula = formula_Sep_sm_detrendlog,  data = temp)
summary(glm.fit3)
...
glm.fit4<- glm(formula = formula_Sep_sm_detrendlog,  data = temp1)
summary(glm.fit4)
plot(glm.fit4) 
...
glm.fit5<- glm(formula = formula_Sep_sm_detrendlog,  data = temp2)
summary(glm.fit5)
coeftest(glm.fit5)
plot(glm.fit5)
...
####################
## PLM Ergebnisse ##
plm.fit_Sep1 <- plm(formula = update(formula_Sep_sm_detrendlog, .~. - dummy(comId)),  data = Yield_Covariates_SM_Sep,  effect="individual", model=("within"), index = c("comId","year"))
summary(plm.fit_Sep1)

# no outliers
plm.fit_Sep2 <- plm(formula = update(formula_Sep_sm_detrendlog, .~. - dummy(comId)),  data = temp,  effect="individual", model=("within"), index = c("comId","year"))
summary(plm.fit_Sep2)
...
plm.fit_Sep3 <- plm(formula = update(formula_Sep_sm_detrendlog, .~. - dummy(comId)),  data = temp1,  effect="individual", model=("within"), index = c("comId","year"))
summary(plm.fit_Sep3)
...
plm.fit_Sep4 <- plm(formula = update(formula_Sep_sm_detrendlog, .~. - dummy(comId)),  data = temp2,  effect="individual", model=("within"), index = c("comId","year"))
summary(plm.fit_Sep4)
...
Mehr noch: da es keine Unterschiede zwischen plm.fit_Sep3 und plm.fit_Sep2 zu geben scheint, macht es wohl Sinn nur die outlier raus zu nehmen. 
...
mit leverage größer 1 rausnehme: plm.fit_Sep3 '
...
plm.fit_Sep_noPrec <- plm(formula = update(formula_Sep_sm_detrendlog, .~. - dummy(comId) - poly(Prec, degree[r, 1], raw = T)),  data = temp1,  effect="individual", model=("within"), index = c("comId","year"))
summary(plm.fit_Sep_noPrec)

## Waldtest with heteroskedasdicity adapted variance covariance matrix
waldtest(plm.fit_Sep3 , plm.fit_Sep_noPrec, vcov= function(x) vcovHC(plm.fit_Sep2, method = "arellano", , type = "HC4"))
...
bptest(plm.fit_Sep, studentize = TRUE)
...
coeftest(plm.fit_Sep)
coeftest(plm.fit_Sep3,vcov=vcovHC)
' hier werden die Standardfehler teilweise sogar kleiner'


## Test ob andere Configuration heteroskedasdicity ausweisen
coeftest(plm.fit_Sep2,vcov=vcovHC(plm.fit_Sep1, method = "arellano", , type = "HC4"))
coeftest(plm.fit_Sep2,vcov=vcovHC(plm.fit_Sep2, method = "arellano", , type = "HC4"))
coeftest(plm.fit_Sep3,vcov=vcovHC(plm.fit_Sep3, method = "arellano", , type = "HC4")) 

coeftest(plm.fit_Sep3,vcov=pvcovHC)
coeftest(plm.fit_Sep3,vcov=vcovHC(plm.fit_Sep2, type = "HC3"))

' Ich denke es macht Sinn das Modell 3 (plm.fit_Sep3 ) ohne outlier und weniger leverage zu nehmen. Darüberhinaus nehme ich arrelano als methode. Der Grund ist, dass durch 
...
''
coeftest(plm.fit_Sep,vcov=vcovHC(plm.fit_Sep, method = "arellano", , type = "HC4")) 


#########################################
## Tests for cross sectiom correlation ##
pcdtest(plm.fit_Sep, test = c("lm"),index = c("comId","year"))
...
# Driscoll Kraay:
coeftest(plm.fit_Sep,vcov=function(x) vcovSCC(plm.fit_Sep,type = "HC4"), cluster="comId", maxlag=0, wj=function(j, maxlag) 1-j/(maxlag+1)) 
...
coeftest(plm.fit_Sep,vcov = vcovDC(plm.fit_Sep,type = "HC4"))
...
summary(m_geo)
summary(plm.fit_Sep) # non clustered stanard error
coeftest(plm.fit_Sep, vcov=vcovHC(plm.fit_Sep)) # clustered standard errors, but no 
...
# BIC_noprec <- rep(0,3)
# for(r in 1:3){
#   glm.fit5 <- glm(formula = update(formula_Sep_sm_detrendlog, .~. - poly(Prec_Sep, degree[r, 1], raw = T)),  data = temp2) 
...
# #### Bootstrap ####
#########################################
# boot.fn = function (data ,index)  return(coef(  plm(formula = update(formula_Sep_sm_detrendlog, .~. - dummy(comId)),  data = temp2,  effect="individual", model=("within"), index = c("comId","year"))  ))
# 
# dim(temp2)
...
save("plm.fit_Sep3 ", file="/home/peichl/Documents/projects/correlation/script/script_raw/Crossvalidation/Output_Sep.RData")
...
names(plm.fit_Sep3)
fixef(plm.fit_Sep3)
hist(fixef(plm.fit_Sep))
plot(fixef(plm.fit_Sep)) # es gibt hier definitiv clustering, das sollte eventuell räumlich dargestellt werden. Vor allem, wenn sich dies bei den anderen Daten wiederholen sollte. 
...
class(fixef(plm.fit_Sep))
str(fixef(plm.fit_Sep))
fixef <- as.data.frame(as.matrix(fixef(plm.fit_Sep)))
# plot(fixef)
str(fixef)
...
## Temperature ##
coeftest(plm.fit_Sep,vcov=vcovHC(plm.fit_Sep3, method = "arellano", , type = "HC4"))
b3 <- coeftest(plm.fit_Sep3,vcov=vcovHC(plm.fit_Sep3, method = "arellano", , type = "HC4"))[3]
b4 <- coeftest(plm.fit_Sep3,vcov=vcovHC(plm.fit_Sep3, method = "arellano", , type = "HC4"))[4]
b5 <- coeftest(plm.fit_Sep3,vcov=vcovHC(plm.fit_Sep3, method = "arellano", , type = "HC4"))[5]
b3
b4
b5
...
## Exp
plot(Yield_Covariates_SM_Sep$Tavg, Yield_Covariates_SM_Sep$siloMaize- mean(Yield_Covariates_SM_Sep$siloMaize), main="Fitted Polynom of Temperature - Exp")
summary(Yield_Covariates_SM_Sep$Tavg)
xcurve <- seq(8.967, 17.4, 0.1)
ycurve <- exp( b3 * xcurve  + b4 * xcurve^2   + b5 * xcurve^3)
lines(xcurve,ycurve, col="green", lwd = 2)

## Precipitation
b1 <-coeftest(plm.fit_Sep3,vcov=vcovHC(plm.fit_Sep3, method = "arellano", , type = "HC4"))[1]
b2 <-  coeftest(plm.fit_Sep3,vcov=vcovHC(plm.fit_Sep3, method = "arellano", , type = "HC4"))[2]

## Log
plot(Yield_Covariates_SM_Sep$Prec, log(Yield_Covariates_SM_Sep$siloMaize - mean(Yield_Covariates_SM_Sep$siloMaize)), main="Fitted Polynom of Precipitation - Log")

summary(Yield_Covariates_SM_Sep$Prec)
...
## Exp
plot(Yield_Covariates_SM_Sep$Prec, Yield_Covariates_SM_Sep$siloMaize- mean(Yield_Covariates_SM_Sep$siloMaize), main="Fitted Polynom of Precipitation -Exp")
# plot(Yield_Covariates_SM_Sep$Prec, Yield_Covariates_SM_Sep$siloMaize, main="Fitted Polynom of Precipitation")
...
b6 <- coeftest(plm.fit_Sep3,vcov=vcovHC(plm.fit_Sep3, method = "arellano", , type = "HC4"))[6]
b7 <- coeftest(plm.fit_Sep3,vcov=vcovHC(plm.fit_Sep3, method = "arellano", , type = "HC4"))[7]
b8 <- coeftest(plm.fit_Sep3,vcov=vcovHC(plm.fit_Sep3, method = "arellano", , type = "HC4"))[8]
b9<- coeftest(plm.fit_Sep3,vcov=vcovHC(plm.fit_Sep3, method = "arellano", , type = "HC4"))[9]
b10 <- coeftest(plm.fit_Sep3,vcov=vcovHC(plm.fit_Sep3, method = "arellano", , type = "HC4"))[10]
b11 <- coeftest(plm.fit_Sep3,vcov=vcovHC(plm.fit_Sep3, method = "arellano", , type = "HC4"))[11]
100*(exp(b6)-1)
100*(exp(b7)-1)
100*(exp(b8)-1)
100*(exp(b9)-1)
...
## Yield vs SMI ##
plot(temp1$SMI, SM_Sep$siloMaize_logtrend- mean(Yield_Covariates_SM_Sep$siloMaize_logtrend), main="Yield vs SMI")
...
plot(Yield_Covariates_SM_Sep$siloMaize ~ as.integer(Yield_Covariates_SM_Sep$year))
lineartrend <- lm(siloMaize ~ as.integer(year), data= Yield_Covariates_SM_Sep )
summary(lineartrend)
abline(lineartrend)
abline(453.5511,0, col="green")
length(resid(lineartrend))
Yield_Covariates_SM_Sep$siloMaize_lintren <- resid(lineartrend)

r=6
formula_Sep_sm_lintren  <- siloMaize_lintren ~ poly(Prec, degree[r, 1], raw = T) +  poly(Tavg, degree[r, 2], raw = T) + 
  dummy(SMI_GDM,c("severe drought","moderate drought","abnormal dry", "abnormal wet", "abundant wet","severe wet")) 

## Remove log trend ##
'Fit log of yield on log of time and use the residuals of that for yields'
plot(log(Yield_Covariates_SM_Sep$siloMaize) ~ log(as.integer(Yield_Covariates_SM_Sep$year)))
logtrend <- lm(log(siloMaize) ~ log(as.integer(year)), data= Yield_Covariates_SM_Sep)
abline(logtrend)
summary(logtrend)
...
r=6
formula_Sep_sm_detrendlog <- siloMaize_logtrend ~ poly(Prec, degree[r, 1], raw = T) +  poly(Tavg, degree[r, 2], raw = T) + 
  dummy(SMI_GDM,c("severe drought","moderate drought","abnormal dry", "abnormal wet", "abundant wet","severe wet")) 
...
for(r in 1:9){
  glm.fit_logdetrend <- glm(formula= update(formula_Sep_sm_detrendlog, .~. + dummy(comId)),  data = Yield_Covariates_SM_Sep) 
...
## PLM Ergebnisse ##
r = r_logdetrend
plm.fit_Sep_detrendlinear <- plm(formula= formula_Sep_sm_detrendlinear,  data = Yield_Covariates_SM_Sep, effect="individual", model="within")
plm.fit_Sep_detrendlog <- plm(formula= formula_Sep_sm_detrendlog,  data = Yield_Covariates_SM_Sep, effect="individual", model="within")

summary(plm.fit_Sep)
summary(plm.fit_Sep_detrendlinear)
summary(plm.fit_Sep_detrendlog)

coeftest(plm.fit_Sep,vcov=vcovHC(plm.fit_Sep, method = "arellano", , type = "HC4"))
coeftest(plm.fit_Sep_detrendlinear,vcov=vcovHC(plm.fit_Sep_detrendlinear, method = "arellano", , type = "HC4"))
coeftest(plm.fit_Sep_detrendlog,vcov=vcovHC(plm.fit_Sep_detrendlog, method = "arellano", , type = "HC4"))

## Gleiche Konfiguration aber andere Jahre: 1:12 statt 1999 - 2010
head(Yield_Covariates_SM_Sep)
Yield_Covariates_SM_Sep$yearshort <- as.integer(Yield_Covariates_SM_Sep$year) - 1999
...
#############################
## Plot SMI Pet ##
plot(Yield_Covariates_SM_Sep$siloMaize ~ Yield_Covariates_SM_Sep$Pet)
plot(Yield_Covariates_SM_Sep$siloMaize ~ Yield_Covariates_SM_Sep$Tavg)

## Determine formula
formula_Sep_sm_Pet <- log(siloMaize) ~ poly(Prec, degree[r, 1], raw = T) +  poly(Pet, degree[r, 2], raw = T) + 
  dummy(SMI_GDM,c("severe drought","moderate drought","abnormal dry", "abnormal wet", "abundant wet","severe wet")) + log(as.integer(year))
formula_Sep_sm_Pet_detrendlog <- siloMaize_logtrend ~ poly(Prec, degree[r, 1], raw = T) +  poly(Pet, degree[r, 2], raw = T) + 
  dummy(SMI_GDM,c("severe drought","moderate drought","abnormal dry", "abnormal wet", "abundant wet","severe wet")) 
...
for(r in 1:9){
  glm.fit_Pet <- glm(formula= update(formula_Sep_sm_Pet_detrendlog, .~. + dummy(comId)),  data = temp1) 
  BIC_Pet[r] <- BIC(glm.fit_Pet)
}
...
r_Pet = 4 
lm.fit_Pet <- lm(formula= update(formula_Sep_sm_Pet_detrendlog, .~. + dummy(comId)),  data = temp1) 
...
## PLM Ergebnisse ##
head(temp1)
plm.fit_Sep_Pet <- plm(formula= formula_Sep_sm_Pet,  data = temp1, effect="individual", model="within")
plm.fit_Sep_Pet_detrendlog <- plm(formula= formula_Sep_sm_Pet_detrendlog,  data = temp1, effect="individual", model="within")

summary(plm.fit_Sep)

summary(plm.fit_Sep_Pet) # R² ist größer, aber Anteil von SMI ist kleiner.
summary(plm.fit_Sep_Pet_detrendlog)
summary(plm.fit_Sep_detrendlog)

coeftest(plm.fit_Sep_Pet_detrendlog,vcov=vcovHC(plm.fit_Sep_Pet_detrendlog, method = "arellano", , type = "HC4"))
...
library(lmtest)
library(car)
bptest(plm.fit_Sep_Pet_detrendlog) # Null, dass es keine heteroskedasdicity gibt, wurde zurückgewiesen. 

coeftest(glm.fit_Pet,vcov=vcovHC(glm.fit_Pet)) # funktioniert nicht, wohl zu viele Dummies
coeftest(plm.fit_Sep_Pet_detrendlog,vcov=vcovHC(plm.fit_Sep_Pet_detrendlog, method = "arellano", , type = "HC4"))
coeftest(plm.fit_Sep_Pet_detrendlog,vcov=vcovHC(plm.fit_Sep_Pet_detrendlog, type = "HC3"))
