...
################
################
names_Aug <- grep(c("*Aug"), names)
names_Aug

Yield_Covariates_Aug <- Yield_Covariates[,names_Aug]
names(Yield_Covariates_Aug)
Yield_Covariates_Aug$silo_Maize <- NULL  # Nur bei Silomaize im Mai notwendig
...
names(Yield_Covariates_SM_Aug) <- c( "comId" , "year","com","idState","State","siloMaize","SMI", "Prec","Tavg", "Pet")
names(Yield_Covariates_SM_Aug)


#################################
#### Define comId as factors ####
class(Yield_Covariates_SM_Aug$comId)
Yield_Covariates_SM_Aug$comId <- as.factor(Yield_Covariates_SM_Aug$comId )
head(Yield_Covariates_SM_Aug$comId)
...
plot(log(Yield_Covariates_SM_Aug$siloMaize) ~ log(as.integer(Yield_Covariates_SM_Aug$year)))
...
formula_Aug_sm_detrendlog <- siloMaize_logtrend ~ poly(Prec, degree[r, 1], raw = T) +  poly(Tavg, degree[r, 2], raw = T) + 
  dummy(SMI_GDM,c("severe drought","moderate drought","abnormal dry", "abnormal wet", "abundant wet","severe wet")) + dummy(comId)

## Print formula
formula_Aug_sm_detrendlog
...
for(r in 1:9){
  glm.fit <- glm(formula = formula_Aug_sm_detrendlog,  data = Yield_Covariates_SM_Aug) 
...
which.min(BIC)
# which.min(cv.error_loocv_Aug_SiloMaize) 
...
####################
## GLM Ergebnisse ##
glm.fit2 <- glm(formula = formula_Aug_sm_detrendlog,  data =  Yield_Covariates_SM_Aug) 
...
dim( Yield_Covariates_SM_Aug)
head(Yield_Covariates_SM_Aug)
summary(Yield_Covariates_SM_Aug$siloMaize)
hist(Yield_Covariates_SM_Aug$siloMaize)


## Generate data without outliers rstd > |6|
na.omit(Yield_Covariates_SM_Aug[rstd > 6 | rstd < -6,])  #11)

Yield_Covariates_SM_Aug[1317,]
Yield_Covariates_SM_Aug[771,]
Yield_Covariates_SM_Aug[847,]
...
Yield_Covariates_SM_Aug[Yield_Covariates_SM_Aug$comId == "5315",] # hier scheint nur ein Jahr ein Messfehler zu sein
Yield_Covariates_SM_Aug[Yield_Covariates_SM_Aug$comId == "5378",] # sollte ich insgesamt rausnehmen, da die Meldungen nicht valide scheinen. 
Yield_Covariates_SM_Aug[Yield_Covariates_SM_Aug$comId == "6532",] # hier scheint nur ein Jahr ein Messfehler zu sein 
Yield_Covariates_SM_Aug[Yield_Covariates_SM_Aug$comId == "12052",] # unvollsätändige Berichterstattung, sollte ich insgesamt rausnehmen
Yield_Covariates_SM_Aug[Yield_Covariates_SM_Aug$comId == "12053",] # hier scheint nur ein Jahr ein Messfehler zu sein
...
na.omit(Yield_Covariates_SM_Aug[rstd > 6 | rstd < -6,])  
dim(Yield_Covariates_SM_Aug)
temp <-  Yield_Covariates_SM_Aug[!rstd > 6 & !rstd < -6,]
dim(temp)
...
r=5
glm.fit3 <- glm(formula = formula_Aug_sm_detrendlog,  data = temp)
summary(glm.fit3)
...
plot(glm.fit3) # Observations with leverage larger than one:852
...
temp[c( 852),]
# Explore comIDs
temp[temp$comId == "5512",] # jeweils nur eine Beobachtung 763
...
glm.fit4<- glm(formula = formula_Aug_sm_detrendlog,  data = temp1)
summary(glm.fit4)
plot(glm.fit4) 
...
glm.fit5<- glm(formula = formula_Aug_sm_detrendlog,  data = temp2)
summary(glm.fit5)
coeftest(glm.fit5)
...
####################
## PLM Ergebnisse ##
plm.fit_Aug1 <- plm(formula = update(formula_Aug_sm_detrendlog, .~. - dummy(comId)),  data = Yield_Covariates_SM_Aug,  effect="individual", model=("within"), index = c("comId","year"))
summary(plm.fit_Aug1)

# no outliers
plm.fit_Aug2 <- plm(formula = update(formula_Aug_sm_detrendlog, .~. - dummy(comId)),  data = temp,  effect="individual", model=("within"), index = c("comId","year"))
summary(plm.fit_Aug2)
...
plm.fit_Aug3 <- plm(formula = update(formula_Aug_sm_detrendlog, .~. - dummy(comId)),  data = temp1,  effect="individual", model=("within"), index = c("comId","year"))
summary(plm.fit_Aug3)
...
plm.fit_Aug4 <- plm(formula = update(formula_Aug_sm_detrendlog, .~. - dummy(comId)),  data = temp2,  effect="individual", model=("within"), index = c("comId","year"))
summary(plm.fit_Aug4)
...
Mehr noch: da es keine Unterschiede zwischen plm.fit_Aug3 und plm.fit_Aug2 zu geben scheint, macht es wohl Sinn nur die outlier raus zu nehmen. 
...
plm.fit_Aug_noPrec <- plm(formula = update(formula_Aug_sm_detrendlog, .~. - dummy(comId) - poly(Prec, degree[r, 1], raw = T)),  data = temp2,  effect="individual", model=("within"), index = c("comId","year"))
summary(plm.fit_Aug_noPrec)

## Waldtest with heteroskedasdicity adapted variance covariance matrix
waldtest(plm.fit_Aug4 , plm.fit_Aug_noPrec, vcov= function(x) vcovHC(plm.fit_Aug4, method = "arellano", , type = "HC4"))
...
bptest(plm.fit_Aug, studentize = TRUE)
...
coeftest(plm.fit_Aug)
coeftest(plm.fit_Aug3,vcov=vcovHC)
' hier werden die Standardfehler teilweise sogar kleiner'


## Test ob andere Configuration heteroskedasdicity ausweisen
coeftest(plm.fit_Aug2,vcov=vcovHC(plm.fit_Aug1, method = "arellano", , type = "HC4"))
coeftest(plm.fit_Aug2,vcov=vcovHC(plm.fit_Aug2, method = "arellano", , type = "HC4"))
coeftest(plm.fit_Aug3,vcov=vcovHC(plm.fit_Aug3, method = "arellano", , type = "HC4")) 

coeftest(plm.fit_Aug3,vcov=pvcovHC)
coeftest(plm.fit_Aug3,vcov=vcovHC(plm.fit_Aug2, type = "HC3"))

' Ich denke es macht Sinn das Modell 3 (plm.fit_Aug3 ) ohne outlier und weniger leverage zu nehmen. Darüberhinaus nehme ich arrelano als methode. Der Grund ist, dass durch 
...
''
coeftest(plm.fit_Aug,vcov=vcovHC(plm.fit_Aug, method = "arellano", , type = "HC4")) 


#########################################
## Tests for cross sectiom correlation ##
pcdtest(plm.fit_Aug, test = c("lm"),index = c("comId","year"))
...
# Driscoll Kraay:
coeftest(plm.fit_Aug,vcov=function(x) vcovSCC(plm.fit_Aug,type = "HC4"), cluster="comId", maxlag=0, wj=function(j, maxlag) 1-j/(maxlag+1)) 
...
coeftest(plm.fit_Aug,vcov = vcovDC(plm.fit_Aug,type = "HC4"))
...
summary(m_geo)
summary(plm.fit_Aug) # non clustered stanard error
coeftest(plm.fit_Aug, vcov=vcovHC(plm.fit_Aug)) # clustered standard errors, but no 
...
# BIC_noprec <- rep(0,3)
# for(r in 1:3){
#   glm.fit5 <- glm(formula = update(formula_Aug_sm_detrendlog, .~. - poly(Prec_Aug, degree[r, 1], raw = T)),  data = temp2) 
...
# #### Bootstrap ####
#########################################
# boot.fn = function (data ,index)  return(coef(  plm(formula = update(formula_Aug_sm_detrendlog, .~. - dummy(comId)),  data = temp2,  effect="individual", model=("within"), index = c("comId","year"))  ))
# 
# dim(temp2)
...
save("plm.fit_Aug3 ", file="/home/peichl/Documents/projects/correlation/script/script_raw/Crossvalidation/Output_Aug.RData")
...
names(plm.fit_Aug3)
fixef(plm.fit_Aug3)
hist(fixef(plm.fit_Aug))
plot(fixef(plm.fit_Aug)) # es gibt hier definitiv clustering, das sollte eventuell räumlich dargestellt werden. Vor allem, wenn sich dies bei den anderen Daten wiederholen sollte. 
...
class(fixef(plm.fit_Aug))
str(fixef(plm.fit_Aug))
fixef <- as.data.frame(as.matrix(fixef(plm.fit_Aug)))
# plot(fixef)
str(fixef)
...
spplot(KreisPolOGR_fixef, zcol="FE", at=at1, col.regions=colorRampPalette(pal)(length(at1)-1),main="Spatial Distribution of Fixed Effects of _Aug - Silo Maize" )
...
## Temperature ##
coeftest(plm.fit_Aug,vcov=vcovHC(plm.fit_Aug3, method = "arellano", , type = "HC4"))
b3 <- coeftest(plm.fit_Aug3,vcov=vcovHC(plm.fit_Aug3, method = "arellano", , type = "HC4"))[3]
b4 <- coeftest(plm.fit_Aug3,vcov=vcovHC(plm.fit_Aug3, method = "arellano", , type = "HC4"))[4]
b5 <- coeftest(plm.fit_Aug3,vcov=vcovHC(plm.fit_Aug3, method = "arellano", , type = "HC4"))[5]
b3
b4
b5
...
## Exp
plot(Yield_Covariates_SM_Aug$Tavg, Yield_Covariates_SM_Aug$siloMaize- mean(Yield_Covariates_SM_Aug$siloMaize), main="Fitted Polynom of Temperature - Exp")
summary(Yield_Covariates_SM_Aug$Tavg)
xcurve <- seq(8.967, 17.4, 0.1)
ycurve <- exp( b3 * xcurve  + b4 * xcurve^2   + b5 * xcurve^3)
lines(xcurve,ycurve, col="green", lwd = 2)

## Precipitation
b1 <-coeftest(plm.fit_Aug3,vcov=vcovHC(plm.fit_Aug3, method = "arellano", , type = "HC4"))[1]
b2 <-  coeftest(plm.fit_Aug3,vcov=vcovHC(plm.fit_Aug3, method = "arellano", , type = "HC4"))[2]

## Log
plot(Yield_Covariates_SM_Aug$Prec, log(Yield_Covariates_SM_Aug$siloMaize - mean(Yield_Covariates_SM_Aug$siloMaize)), main="Fitted Polynom of Precipitation - Log")

summary(Yield_Covariates_SM_Aug$Prec)
...
## Exp
plot(Yield_Covariates_SM_Aug$Prec, Yield_Covariates_SM_Aug$siloMaize- mean(Yield_Covariates_SM_Aug$siloMaize), main="Fitted Polynom of Precipitation -Exp")
# plot(Yield_Covariates_SM_Aug$Prec, Yield_Covariates_SM_Aug$siloMaize, main="Fitted Polynom of Precipitation")
...
b6 <- coeftest(plm.fit_Aug3,vcov=vcovHC(plm.fit_Aug3, method = "arellano", , type = "HC4"))[6]
b7 <- coeftest(plm.fit_Aug3,vcov=vcovHC(plm.fit_Aug3, method = "arellano", , type = "HC4"))[7]
b8 <- coeftest(plm.fit_Aug3,vcov=vcovHC(plm.fit_Aug3, method = "arellano", , type = "HC4"))[8]
b9<- coeftest(plm.fit_Aug3,vcov=vcovHC(plm.fit_Aug3, method = "arellano", , type = "HC4"))[9]
b10 <- coeftest(plm.fit_Aug3,vcov=vcovHC(plm.fit_Aug3, method = "arellano", , type = "HC4"))[10]
b11 <- coeftest(plm.fit_Aug3,vcov=vcovHC(plm.fit_Aug3, method = "arellano", , type = "HC4"))[11]
100*(exp(b6)-1)
100*(exp(b7)-1)
100*(exp(b8)-1)
100*(exp(b9)-1)
...
## Yield vs SMI ##
plot(temp1$SMI, SM_Aug$siloMaize_logtrend- mean(Yield_Covariates_SM_Aug$siloMaize_logtrend), main="Yield vs SMI")
...
plot(Yield_Covariates_SM_Aug$siloMaize ~ as.integer(Yield_Covariates_SM_Aug$year))
lineartrend <- lm(siloMaize ~ as.integer(year), data= Yield_Covariates_SM_Aug )
summary(lineartrend)
abline(lineartrend)
abline(453.5511,0, col="green")
length(resid(lineartrend))
Yield_Covariates_SM_Aug$siloMaize_lintren <- resid(lineartrend)

r=6
formula_Aug_sm_lintren  <- siloMaize_lintren ~ poly(Prec, degree[r, 1], raw = T) +  poly(Tavg, degree[r, 2], raw = T) + 
...
for(r in 1:9){
  glm.fit_logdetrend <- glm(formula= update(formula_Aug_sm_detrendlog, .~. + dummy(comId)),  data = Yield_Covariates_SM_Aug) 
...
## PLM Ergebnisse ##
r = r_logdetrend
plm.fit_Aug_detrendlinear <- plm(formula= formula_Aug_sm_detrendlinear,  data = Yield_Covariates_SM_Aug, effect="individual", model="within")
plm.fit_Aug_detrendlog <- plm(formula= formula_Aug_sm_detrendlog,  data = Yield_Covariates_SM_Aug, effect="individual", model="within")

summary(plm.fit_Aug)
summary(plm.fit_Aug_detrendlinear)
summary(plm.fit_Aug_detrendlog)

coeftest(plm.fit_Aug,vcov=vcovHC(plm.fit_Aug, method = "arellano", , type = "HC4"))
coeftest(plm.fit_Aug_detrendlinear,vcov=vcovHC(plm.fit_Aug_detrendlinear, method = "arellano", , type = "HC4"))
coeftest(plm.fit_Aug_detrendlog,vcov=vcovHC(plm.fit_Aug_detrendlog, method = "arellano", , type = "HC4"))

## Gleiche Konfiguration aber andere Jahre: 1:12 statt 1999 - 2010
head(Yield_Covariates_SM_Aug)
Yield_Covariates_SM_Aug$yearshort <- as.integer(Yield_Covariates_SM_Aug$year) - 1999
...
#############################
## Plot SMI Pet ##
plot(Yield_Covariates_SM_Aug$siloMaize ~ Yield_Covariates_SM_Aug$Pet)
plot(Yield_Covariates_SM_Aug$siloMaize ~ Yield_Covariates_SM_Aug$Tavg)

## Determine formula
formula_Aug_sm_Pet <- log(siloMaize) ~ poly(Prec, degree[r, 1], raw = T) +  poly(Pet, degree[r, 2], raw = T) + 
  dummy(SMI_GDM,c("severe drought","moderate drought","abnormal dry", "abnormal wet", "abundant wet","severe wet")) + log(as.integer(year))
formula_Aug_sm_Pet_detrendlog <- siloMaize_logtrend ~ poly(Prec, degree[r, 1], raw = T) +  poly(Pet, degree[r, 2], raw = T) + 
  dummy(SMI_GDM,c("severe drought","moderate drought","abnormal dry", "abnormal wet", "abundant wet","severe wet")) 
...
for(r in 1:9){
  glm.fit_Pet <- glm(formula= update(formula_Aug_sm_Pet_detrendlog, .~. + dummy(comId)),  data = temp1) 
...
r_Pet = 4 
lm.fit_Pet <- lm(formula= update(formula_Aug_sm_Pet_detrendlog, .~. + dummy(comId)),  data = temp1) 
...
## PLM Ergebnisse ##
head(temp1)
plm.fit_Aug_Pet <- plm(formula= formula_Aug_sm_Pet,  data = temp1, effect="individual", model="within")
plm.fit_Aug_Pet_detrendlog <- plm(formula= formula_Aug_sm_Pet_detrendlog,  data = temp1, effect="individual", model="within")

summary(plm.fit_Aug)

summary(plm.fit_Aug_Pet) # R² ist größer, aber Anteil von SMI ist kleiner.
summary(plm.fit_Aug_Pet_detrendlog)
summary(plm.fit_Aug_detrendlog)

coeftest(plm.fit_Aug_Pet_detrendlog,vcov=vcovHC(plm.fit_Aug_Pet_detrendlog, method = "arellano", , type = "HC4"))
...
library(lmtest)
library(car)
bptest(plm.fit_Aug_Pet_detrendlog) # Null, dass es keine heteroskedasdicity gibt, wurde zurückgewiesen. 

coeftest(glm.fit_Pet,vcov=vcovHC(glm.fit_Pet)) # funktioniert nicht, wohl zu viele Dummies
coeftest(plm.fit_Aug_Pet_detrendlog,vcov=vcovHC(plm.fit_Aug_Pet_detrendlog, method = "arellano", , type = "HC4"))
coeftest(plm.fit_Aug_Pet_detrendlog,vcov=vcovHC(plm.fit_Aug_Pet_detrendlog, type = "HC3"))
...
names(Yield_Covariates)
names <- names(Yield_Covariates)
_Aug <- grep(c("*Jul"), names)
_Aug

Yield_Covariates_Aug <- Yield_Covariates[,_Aug]
names(Yield_Covariates_Aug)
Yield_Covariates_Aug$siloMaize <- NULL  # Nur bei Silomaize im _Aug notwendig
dim(Yield_Covariates_Aug)
...
mit leverage größer 1 rausnehme: plm.fit_Aug3 '
...
# Model ohne Preicpitation
plm.fit_Aug_noPrec <- plm(formula = update(formula_Aug_sm_detrendlog, .~. - dummy(comId) - poly(Prec, degree[r, 1], raw = T)),  data = temp1,  effect="individual", model=("within"), index = c("comId","year"))
summary(plm.fit_Aug_noPrec)

## Waldtest with heteroskedasdicity adapted variance covariance matrix
waldtest(plm.fit_Aug3 , plm.fit_Aug_noPrec, vcov= function(x) vcovHC(plm.fit_Aug2, method = "arellano", , type = "HC4"))
' Es macht Sinn die NiederschlagsVariablen im Model zu lassen, da diese auch gemeinsam significant sind.'
...
## Plot Spatial Dataframe without color specification
spplot(KreisPolOGR_fixef, zcol="FE", main="Spatial Distribution of Fixed Effects of _Aug - Silo Maize" )
