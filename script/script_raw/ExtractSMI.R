...
- mSMiMai2003 <- raster layer (4km²)
- mSMiMai2003Dis <- disaggregate (100m²)
- mSMiMai2003DisMask <- masked (100m²)
...
'clc2006 <- CLC ASCII von Matthias Zink bereitgestellt. 
KreisPolOGR <- Polygone der Kreise von Matthis Zink bereitgestellt
mSMi (4*4 km  bis 2015) <- bereitgestellt von Stephan Thober. Auf dem cluster unter daten/klimabuer/agri zu finden. 
'
#### Packages ####
library(sp)
library(rgdal)
...
#######################################
#### Load Corinne Land Cover Ascii ####
## File ist komprimiert, muss erst entpackt werden
CLCMask<-raster("data/data_raw/4_michael/clc_2006.asc") ### dadurch wird die file viel kleiner:12 kb statt 240 MB; Aber raster setzt nur pointer
CLCMask
table(values(CLCMask))
####################################################################
#### Make mask for values of 12, i.e. non-irrigated arable land ####
## Convert raster to matrix ##
CLC <- as.matrix(CLCMask) # File sehr groß
CLCm <- CLC
## Mask for non-irrigated arable land: CLC 12
u <- CLCm != "12"
CLCm[u] <- NA 
dim(CLCm)

## Include new date in old raster frame ##
values(CLCMask) <- CLCm
CLCMask
...
## Plot Mask ##
CLCMask_plot <- spplot(CLCMask, maxpixels=5000000)
CLCMask_plot

#### Load KreisPolygone ####
KreisPolOGR <- readOGR("/Storage/ownCloud/Home/Klimabuero/Proj1/data//data_raw/4_michael/adminitrative_borders_Ger/", "vg2500_krs")
KreisPolOGR
## Plot mask and Kreispolygones ##
CLCMask_plot + layer(sp.polygons(KreisPolOGR, col="black", lwd=1))


#########################################################################################
#### Laden des SMI (ursprüngliche Version (4*4 km), also keine Maskierung und Disaggregation) ####
mSMi <- brick("./data/data_raw/mSMI_4x4km.nc", varname="SMI")
# # plot(mSMi)
mSMi
...
# projection(mSMi)< "+proj=longlat +datum=WGS84"
...
# #### 0 Werte in NA ####
# any(values(mSMiMai2003) == 0.0000000)
# u <- (values(mSMiMai2003) == 0.0000000)
# values(mSMiMai2003)[u] <- NA
...
d <-  flip(mSMiMai2003, direction = "y")
d
spplot(d)

#####################################
## Change extent to CLCMASK extent ##
extent(d) <- extent(CLCMask)
mSMiMai2003 <- d

spplot((mSMiMai2003))

## Write SMI of Layer 629 ##
writeRaster(mSMiMai2003,"./data/data_raw/mSMiMai2003", overwrite=T) 
...
writeRaster(mSMiMai2003Dis,"./data/data_raw/mSMiMai2003Dis", overwrite=T) 

#### Read disaggregated SMI of Layer 720
mSMiMai2003Dis2 <- raster("./data/data_raw/mSMiMai2003Dis")

## Plot ##
par(mfrow=c(1,2))
spplot(mSMiMai2003, main="SMI Germany, 4000*4000")
spplot(mSMiMai2003Dis, main="SMI Germany, 100*100")
...
writeRaster(mSMiMai2003DisMask,"data/data_raw/mSMiMai2003DisMask",  overwrite=T)
# 
...
projection(mSMiMai2003DisMask) <- "+proj=tmerc +lat_0=0 +lon_0=12 +k=1 +x_0=4500000 +y_0=0 +datum=potsdam +units=m +no_defs +ellps=bessel +towgs84=598.1,73.7,418.2,0.202,0.045,-2.455,6.7"
mSMiMai2003DisMask
mSMiMai2003DisMask_plot <- spplot(mSMiMai2003DisMask)
mSMiMai2003DisMask_plot
mSMiMai2003DisMask_plot  + layer(sp.polygons(KreisPolOGR, col="black", lwd=1))
...
## disaggregated data ##
## Year 2003 ##
mSMiMai2003Dis2003 <- brick("data/data_raw/mSMiSubDisMaskStack2003.nc")
mSMiMai2003Dis  <- raster(mSMiMai2003Dis2003, layer=5)
...
idx_names<-make.names(c(idx_gsub), unique = TRUE)
idx_names
length(idx_names)==length(names(mSMiMai2003Dis2003))
...
## Setuo Colourscheme ##
cs <-colorRampPalette(c("tomato4","tan4", "chartreuse4","white","chartreuse4","dodgerblue4", "darkblue"))(1e3)
...
mSMi2003Plt<-spplot(mSMiMai2003 ,at=at2, col.regions= cs)
mSMi2003Plt
