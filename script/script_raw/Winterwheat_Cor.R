...
- demanding on soil quality, climate and water supply    
- Licht & Temperatur (Temp), Wasser (SMI, Niederschlag), Boden (SMI, FE)
...
Metodisches Vorgehen
  ...
  - zwei Unterschiedliche Motivationen (CCFS6)
      - forecast as yield at a given place in year under existing weather conditions: 
        time series: assumes farmer make optimal decisions according to existing distribution of 
  ...
  - Lineare Panel Daten Modelle (generall kann man sagen, dass das die mehr ökonomische Perspektive , siehe Anwendungen von Wolfram Schlenker)
      ...
      - Fixed Effects
          ...
          - (CCFS6): two ways of calculation fe-coefficients
                          ...
                          - demean each variablen (dependent and independent) and  run linear regression trough them
      ...
      - Fixed vs Random Effect
          ...
          - Hausmann Test:  null hypothesis is that the preferred model is random effects vs. the alternative the fixed effects; It basically tests whether the unique
                            errors (u_i) are correlated with the regressors, the null hypothesis is they are not -> Null rejected: FE
...
Drainage:
  - Eventuell müssen Geegenden/Crops mit hohen Drainage Anteil rausgerechnet werden.
...
r <- names(Yield_Covariates_nodemean_ww)
rr <- grep("*zscore", r, invert=T)
Yield_Covariates_nodemean_nozscore_ww <- Yield_Covariates_nodemean_ww[,rr]
dim(Yield_Covariates_nodemean_nozscore_ww)
class(Yield_Covariates_nodemean_nozscore_ww)
head(Yield_Covariates_nodemean_nozscore_ww)
unique(Yield_Covariates_nodemean_nozscore_ww$year)
sum(is.na(Yield_Covariates_nodemean_nozscore_ww)) 

dim(Yield_Covariates_nodemean_nozscore_ww)
is.na(Yield_Covariates_nodemean_nozscore_ww)

names(Yield_Covariates_nodemean_nozscore_ww)
any(is.na(Yield_Covariates_nodemean_nozscore_ww$comIdState))
any(is.na(Yield_Covariates_nodemean_nozscore_ww$comState))

sum(is.na(Yield_Covariates_nodemean_nozscore_ww$area100ha)) # Nas


sum(is.na(Yield_Covariates_nodemean_nozscore_ww$SHAPE_AREA)) # no Nas


sum(is.na(Yield_Covariates_nodemean_nozscore_ww$winterWheat)) # 668
sum(is.na(Yield_Covariates_nodemean_nozscore_ww$winterWheat_deZscore)) # 900
sum(is.na(Yield_Covariates_nodemean_nozscore_ww$Tavg_Feb)) # 48/12 = 4 -> Das sind genau die vier #319 (Oberallgäu),315 (Lindau),308 (Kempten),236( Garmisch Partenkirchen)
sum(is.na(Yield_Covariates_nodemean_nozscore_ww$Por_L1L2_30))
sum(is.na(Yield_Covariates_nodemean_nozscore_ww$Por_L3_nd))
sum(is.na(Yield_Covariates_nodemean_nozscore_ww$SMI_Sep)) # 48
sum(is.na(Yield_Covariates_nodemean_nozscore_ww$SMI_Sep_lag)) #48

sum(is.na(Yield_Covariates_nodemean_nozscore_ww$year))

all(is.na(Yield_Covariates_nodemean_nozscore_ww$Tavg_Feb) == is.na(Yield_Covariates_nodemean_nozscore_ww$SMI_Feb))
all(is.na(Yield_Covariates_nodemean_nozscore_ww$Tavg_Feb) == is.na(Yield_Covariates_nodemean_nozscore_ww$Prec_Jan))    
all(is.na(Yield_Covariates_nodemean_nozscore_ww$Tavg_Feb) == is.na(Yield_Covariates_nodemean_nozscore_ww$Pet_Apr))    
all(is.na(Yield_Covariates_nodemean_nozscore_ww$Tavg_Feb) == is.na(Yield_Covariates_nodemean_nozscore_ww$Por_L1_5))    
all(is.na(Yield_Covariates_nodemean_nozscore_ww$Tavg_Feb) == is.na(Yield_Covariates_nodemean_nozscore_ww$Tavg_Mai))  
all(is.na(Yield_Covariates_nodemean_nozscore_ww$Tavg_Feb) == is.na(Yield_Covariates_nodemean_nozscore_ww$Tmin_Jun))  
all(is.na(Yield_Covariates_nodemean_nozscore_ww$Tavg_Feb) == is.na(Yield_Covariates_nodemean_nozscore_ww$Tmax_Jul))  
all(is.na(Yield_Covariates_nodemean_nozscore_ww$Tavg_Feb) == is.na(Yield_Covariates_nodemean_nozscore_ww$SMI_Sep_lag))  

Yield_Covariates_nodemean_nozscore_ww$area100ha <- Yield_Covariates_nodemean_nozscore_ww$winterWheat_deZscore <- NULL

Yield_Covariates <- Yield_Covariates_nodemean_nozscore_ww
names(Yield_Covariates)

#########################################################
#### Write Yield_SMI_Prec_Tavg_Pet_Dem_Por_Tmin_Tmax ####
#########################################################
write.csv(Yield_Covariates, "data/data_processed/Yield_SMI_Prec_Tavg_Pet_Dem_Por_Tmin_Tmax_nodemean_nozscore_ww.csv")
...
summary(winterWheat)
plot(winterWheat ~ year)

## 
any(winterWheat == 0, na.rm=T)
...
##########################################################################
## Calculate Growth Months of Winterwheat: nach Schindler April to June ##
# ## average
...
# #####################################################################################
# ## Calculate Growth Months of Winterwheat: nach Schlenker in den USA March to Aug ##
# ## average
...
trend
plot(detrend ~ year)
detrend <- detrend(winterWheat, 'linear') 
abline(trend)
summary(detrend)
...
names(Yield_Covariates)
idvars = c("comId", "year","comIdState","comState","winterWheat")
...
qq_boxplot_winterWheat  <- ggplot(Yield_Covariates, aes_string("year", "winterWheat")) +
#                               geom_boxplot() +
                              geom_jitter() +
...
qq_boxplot_scoreWW  <- ggplot(Yield_Covariates, aes_string("year", "winterWheat_deZscore")) +
#   geom_boxplot() +
  geom_jitter() +
...
qq_boxplot_winterWheat_deZscore <- ggplot(Yield_Covariates, aes_string("year", "winterWheat_deZscore")) +
  #                               geom_boxplot() +
  geom_jitter() +
  geom_smooth(method = "lm", se=F, color="green3", size=2, aes(group=1)) +
  geom_smooth(se=F, color="red", size=2, aes(group=1))+
...
qp_Yearly_MaiToJul_lin<-qplot(SMI_Yearly_MaiToJul, winterWheat, data=Yield_Covariates, facets=~year, color=year,geom=c("point", "smooth"), method="lm")+ ylab("Yearly Winter Wheat Yield in dt/ha") + ylab("Yearly Winter Wheat Yield in dt/ha") + ggtitle("Winterwheat, Linear, SMI_Yearly_MaiToJul")
...
qp_Yearly_MaiToJul_pol<-qplot(SMI_Yearly_MaiToJul, winterWheat, data=Yield_Covariates, facets=~year, color=factor(year),geom=c("point", "smooth"),shape=".")+ ylab("Yearly Winter Wheat Yield in dt/ha") + ylab("Yearly Winter Wheat Yield in dt/ha") + ggtitle("Winterwheat, Pol, SMI_Yearly_MaiToJul")
...
# #### plotmeans ####
# plotmeans(winterWheat ~ comId, Yield_Covariates)
# plotmeans(winterWheat ~ comState, Yield_Covariates)
# plotmeans(winterWheat ~ year, Yield_Covariates)
...
gg_lin_months  <-lapply(SMI_month_list[[1]] , function(i)  ggplot(Yield_Covariates, aes_string(x=i,y="winterWheat")) +  stat_smooth(method="lm", se=F ) + geom_point(shape = 5, size = 0.5)          
                        + ylab ("Per acre yield of Winterwheat") + ggtitle( "xy-Plot") + theme_bw() )
gg_lin_months 

###########################################################
## Linear mit unterschiedlichen fitted lines nach Jahren ##
gg_lin_months  <-lapply(SMI_month_list[[1]] , function(i)  ggplot(Yield_Covariates, aes_string(x=i,y="winterWheat", color="year")) +  stat_smooth(method="lm", se=F ) + geom_point(shape = 5, size = 0.5)          
                        + ylab ("Per acre yield of Winterwheat") + ggtitle( "xy-Plot") + theme_bw() )
gg_lin_months 
...
## Polygons
gg_pol_months_yearColor <- lapply(SMI_month_list[[1]], function(i)  ggplot(Yield_Covariates, aes_string(x=i,y="winterWheat", color="year")) +  stat_smooth(se=F) + geom_point(shape = 5, size = 0.5)          
                     + ylab ("Per acre yield of Winterwheat")  + ggtitle( "xy-Plot") + theme_bw() )
gg_pol_months_yearColor

## Polygons
gg_pol_months <- lapply(SMI_month_list[[1]], function(i)  ggplot(Yield_Covariates, aes_string(x=i,y="winterWheat")) +  stat_smooth(se=F) + geom_point(shape = 5, size = 0.5)          
                        + ylab ("Per acre yield of Winterwheat")  + ggtitle( "xy-Plot") + theme_bw() )
gg_pol_months
...
## Linear line
gg_lin_months_facetCom  <-lapply(SMI_month_list[[1]], function(i)  ggplot(Yield_Covariates, aes_string(x=i,y="winterWheat")) +  stat_smooth(method="lm", se=F ) + geom_point(shape = 5, size = 0.5)          
                        + ylab ("Per acre yield of Winterwheat")  + ggtitle( "xy-Plot") + theme_bw() + facet_wrap(~comState)) 
gg_lin_months_facetCom[1]

gg_lin_months_facetYear  <-lapply(SMI_month_list[[1]], function(i)  ggplot(Yield_Covariates, aes_string(x=i,y="winterWheat", color="comState")) +  stat_smooth(method="lm", se=F ) + geom_point(shape = 5, size = 0.5)          
                                 + ylab ("Per acre yield of Winterwheat")  + ggtitle( "xy-Plot") + theme_bw() + facet_wrap(~year)) 
gg_lin_months_facetYear[1]

## Polygons

gg_pol_months_facetCom <- lapply(SMI_month_list[[1]], function(i)  ggplot(Yield_Covariates, aes_string(x=i,y="winterWheat")) +  stat_smooth(se=F) + geom_point(shape = 5, size = 0.5)          
                        + ylab ("Per acre yield of Winterwheat")  + ggtitle( "xy-Plot") + theme_bw() + facet_wrap(~comState))
gg_pol_months_facetCom[1]

gg_pol_months_facetYear <- lapply(SMI_month_list[[1]], function(i)  ggplot(Yield_Covariates, aes_string(x=i,y="winterWheat", color="year")) +  stat_smooth( se=F) + geom_point(shape = 5, size = 0.5)          
                                 + ylab ("Per acre yield of Winterwheat")  + ggtitle( "xy-Plot") + theme_bw() + facet_wrap(~year))
...
ggplot_pol_SMI_facetMonths<- ggplot(Yield_Covariates_melt, aes(x=value, y=winterWheat)) + 
...
## facet, linear
ggplot_lin_SMI_facetMonths<- ggplot(Yield_Covariates_melt, aes(x=value, y=winterWheat)) + 
...
# ## facet, non linear
# names(Yield_Covariates_melt)
# ggplot_pol_SMI_facetMonths_l01<- ggplot(Yield_Covariates_melt_l01, aes(x=value, y=winterWheat)) + 
#   stat_smooth(color="red",size = 2, se=T) +
#   geom_point(shape = 5, size = 0.5) +
#   facet_wrap(~variable)+ ylab ("Per acre yield of Winterwheat")  +
#   ggtitle( "xy-Plot") + theme_bw()
...
# ## facet, linear
# names(Yield_Covariates_melt)
# ggplot_lin_SMI_facetMonths_l01<- ggplot(Yield_Covariates_melt_l01, aes(x=value, y=winterWheat)) + 
#   stat_smooth(method="lm",color="red",size = 2, se=T) +
#   geom_point(shape = 5, size = 0.5) +
#   facet_wrap(~variable)+ ylab ("Per acre yield of Winterwheat")  +
#   ggtitle( "xy-Plot") + theme_bw()
...
# 
# ## nofacet, linear
# ggplot_lin_SMI_l01 <- ggplot(Yield_Covariates_melt_l01, aes(x=(value), y=winterWheat)) + 
#   stat_smooth(method="lm",color="red",size = 2, se=T) +
#   geom_point(shape = 5, size = 0.5) +
...
# 
# ## no facet, nonlinear
# ggplot_pol_SMI_l01 <- ggplot(Yield_Covariates_melt_l01, aes(x=(value), y=winterWheat)) + 
#   stat_smooth(color="red",size = 2, se=T) +
#   geom_point(shape = 5, size = 0.5) +
#   ylab ("Per acre yield of Winterwheat")  +
#   ggtitle( "xy-Plot") + theme_bw()
...
# ## facet, non linear
# names(Yield_Covariates_melt)
# ggplot_pol_SMI_facetMonths_m09<- ggplot(Yield_Covariates_melt_m09, aes(x=value, y=winterWheat)) + 
#   stat_smooth(color="red",size = 2, se=T) +
#   geom_point(shape = 5, size = 0.5) +
#   facet_wrap(~variable)+ ylab ("Per acre yield of Winterwheat")  +
#   ggtitle( "xy-Plot") + theme_bw()
...
# ## facet, linear
# names(Yield_Covariates_melt)
# ggplot_lin_SMI_facetMonths_m09<- ggplot(Yield_Covariates_melt_m09, aes(x=value, y=winterWheat)) + 
#   stat_smooth(method="lm",color="red",size = 2, se=T) +
#   geom_point(shape = 5, size = 0.5) +
#   facet_wrap(~variable)+ ylab ("Per acre yield of Winterwheat")  +
#   ggtitle( "xy-Plot") + theme_bw()
...
# 
# ## no facet, non linear
# ggplot_pol_SMI_m09 <- ggplot(Yield_Covariates_melt_m09, aes(x=(value), y=winterWheat)) + 
#   stat_smooth(color="red",size = 2, se=T) +
#   geom_point(shape = 5, size = 0.5) +
#   ylab ("Per acre yield of Winterwheat")  +
#   ggtitle( "xy-Plot") + theme_bw()
...
# 
# ## no facet, linear
# ggplot_lin_SMI_m09 <- ggplot(Yield_Covariates_melt_m09, aes(x=(value), y=winterWheat)) + 
#   stat_smooth(method="lm",color="red",size = 2, se=T) +
#   geom_point(shape = 5, size = 0.5) +
#   ylab ("Per acre yield of Winterwheat")  +
#   ggtitle( "xy-Plot") + theme_bw()
...
## non lag
names(Yield_Covariates_melt)
ggplot_pol_SMI_facetMonths<- ggplot(Yield_Covariates_melt, aes(x=value, y=winterWheat_deZscore)) + 
  stat_smooth(color="red",size = 2, se=T) +
...
## lagged
names(Yield_Covariates_melt)
ggplot_pol_SMI_facetMonths_lag<- ggplot(Yield_Covariates_melt_lag, aes(x=value, y=winterWheat)) + 
...
# 
# ## Plot ##
# qp_YearlyAvg_lin<-qplot(SMI_YearlyAvg, winterWheat, data=Yield_Covariates, facets=~year, color=factor(year),geom=c("point", "smooth"), method="lm",shape=".")+ ylab("Yearly Winter Wheat Yield in dt/ha") + ggtitle("Winterwheat, Linear, SMI_YearlyAvg")
# summary(qp_Aug_lin)
# 
# qp_YearlyAvg_pol<-qplot(SMI_YearlyAvg, winterWheat, data=Yield_Covariates, facets=~year, color=factor(year),geom=c("point", "smooth"),shape=".")+ ylab("Yearly Winter Wheat Yield in dt/ha") + ggtitle("Winterwheat, Pol, SMI_Yearly_Avg")
# summary(qp_Aug_lin)
# 
# 
# #### Plot the time series of each state ####
# qp_timeSeries_comIdState <-qplot(SMI_YearlyAvg, winterWheat, data=Yield_Covariates, facets=~comIdState,  geom=c("point", "smooth"), method="lm",shape=".", se=TRUE)+ ylab("Yearly Winter Wheat Yield in dt/ha") + ggtitle("Winterwheat, Linear, Jan")
# 
# qp_timeSeries_comIdState <-qplot(SMI_Jan, year, data=Yield_Covariates, facets=~comIdState,  geom=c("point", "smooth"), method="lm",shape=".", se=TRUE) + ylab("Yearly Winter Wheat Yield in dt/ha") + ggtitle("Winterwheat, Linear, Jan")
# 
...
Yield_Covariates <- read.csv( "data//data_processed/Yield_SMI_Prec_Tavg_Pet_Dem_Por_Tmin_Tmax_nodemean_nozscore_ww.csv")
...
4920-4215
' The difference is rational, because there are around 668 missing winterWheat values and 48 SMI, which are not necessarily orthogonal'
...
class(Yield_Covariates_nna_plm$winterWheat)
names(Yield_Covariates_nna_plm)
str(Yield_Covariates_nna_plm)
str(Yield_Covariates_nna_plm$winterWheat)
...
any(winterWheat==0) # can use log transformation;
...
## Modell Estimation ##
names(Yield_Covariates_nna_plm)
summary(log(winterWheat))

## Pooled: Basic linear model, short, non poly, non transform, average ##
Pool_SMI_GP_short <-lm(formula = log(winterWheat) ~ SMI_GP_short_av)

## Pooled: Basic linear model, long, non poly, non transform, average ##
Pool_SMI_GP_long <-lm(formula = log(winterWheat) ~ SMI_GP_long_av)
...
predict(Pool_SMI_GP_short, data.frame(SMI_GP_short_av = (c(0.2, 0.5, 0.8))), interval = "predict") # prduced prediction intervalls for log(winterWheat predictions for given SMI_GP_short_av)
                                                                                                 # quantifies uncertainity for a particular value of SMI
                                                      
## Plot ##
plot(log(winterWheat) ~ SMI_GP_short_av)
plot(log(winterWheat) ~ SMI_GP_short_av)
plot(winterWheat_deZscore ~ SMI_GP_short_av)
...
## Polygon 3. Grades ##
Pool_SMI_GP_short_poly <-lm(formula = log(winterWheat) ~ poly(SMI_GP_short_av, 3, raw=T))
Pool_SMI_GP_long_poly <-lm(formula = log(winterWheat) ~ poly(SMI_GP_long_av, 3, raw=T))
# Diagnostics
...
FE_SMI_GP_short <-plm(formula = winterWheat ~ SMI_GP_short_av, data = Yield_Covariates_nna_plm, index=c("comId", "year"), model="within")
...
FE_SMI_GP_short_poly <-plm(formula = winterWheat ~ poly(SMI_GP_short_av, 2, raw=T), data = Yield_Covariates_nna_plm, index=c("comId", "year"), model="within")
...
RE_SMI_GP_short_re <-plm(formula = log(winterWheat) ~ SMI_GP_short_av, data = Yield_Covariates_nna_plm, index=c("comId", "year"), model="random")
...
RE_SMI_GP_short_poly_re <-plm(formula = log(winterWheat) ~ poly(SMI_GP_short_av, 2, raw=T), data = Yield_Covariates_nna_plm, index=c("comId", "year"), model="random")
...
## Fixed Effects, basic Model ##
FE_nonSMI_GP_short <-plm(formula = log(winterWheat) ~ Prec_GP_short_av + Tavg_GP_short_av + Pet_GP_short_av,  data = Yield_Covariates_nna_plm, index=c("comId", "year"), model = "within", effect = "twoways")
...
## Fixed Effects, poly2, basic Model ##
FE_nonSMI_poly_GP_short <-plm(formula = log(winterWheat) ~ poly(Prec_GP_short_av, 2, raw=T) + poly(Tavg_GP_short_av, 2, raw=T) + poly(Pet_GP_short_av , 2, raw=T), data = Yield_Covariates_nna_plm, index=c("comId", "year"), model="within")
...
## Random Effects, basic Model ##
RE_nonSMI_GP_short <-plm(formula = log(winterWheat) ~ Prec_GP_short_av + Tavg_GP_short_av + Pet_GP_short_av , data = Yield_Covariates_nna_plm, index=c("comId", "year"), model="random")
...
RE_SMI_GP_short_poly <-plm(formula = log(winterWheat) ~ poly(Prec_GP_short_av, 2, raw=T), data = Yield_Covariates_nna_plm, index=c("comId", "year"), model="random")
...
Pool_TavgPrecPetSmi_GP_short <-lm(formula = winterWheat ~ Tavg_GP_short_av + Prec_GP_short_av + Pet_GP_short_av +  SMI_GP_short_av,  data = Yield_Covariates_nna_plm)
...
Pool_TavgPrecPetSmi_GP_short_log <-lm(formula = log(winterWheat) ~ Tavg_GP_short_av + Prec_GP_short_av + Pet_GP_short_av +  SMI_GP_short_av,  data = Yield_Covariates_nna_plm)
...
Pool_TavgPrecPetSmi_GP_short_zscore <-lm(formula = winterWheat_deZscore ~ Tavg_GP_short_av + Prec_GP_short_av + Pet_GP_short_av +  SMI_GP_short_av,  data = Yield_Covariates_nna_plm)
...
Pool_TavgPrecPetSmi_poly_GP_short <-lm(formula = winterWheat ~ poly(Prec_GP_short_av, 2, raw=T) +  poly(Tavg_GP_short_av, 2, raw=T) + poly(Pet_GP_short_av , 2, raw=T) + poly(SMI_GP_short_av, 2, raw=T),  data = Yield_Covariates_nna_plm)
...
FE_TavgPrecPetSmi_GP_short <-plm(formula = winterWheat_deZscore ~ Prec_GP_short_av + Tavg_GP_short_av + Pet_GP_short_av + SMI_GP_short_av,  data = Yield_Covariates_nna_plm, index=c("comId", "year"), model = "within", effect = "twoways")
...
FE_TavgPrecPetSmi_poly_GP_short <-plm(formula = winterWheat ~ poly(Pet_GP_short_av, 2, raw=T) +    poly(Prec_GP_short_av, 2, raw=T) +  poly(Tavg_GP_short_av, 1, raw=T) + poly(SMI_GP_short_av, 3, raw=T) , data = Yield_Covariates_nna_plm, , index=c("comId", "year"), model="within", effect = "individual")
# Diagnostics
...
FE_TavgPrecPetSmi_poly_GP_short_trend <-plm(formula = winterWheat ~ poly(Pet_GP_short_av, 2, raw=T) +    poly(Prec_GP_short_av, 2, raw=T) +  poly(Tavg_GP_short_av, 1, raw=T) + poly(SMI_GP_short_av, 3, raw=T)  + poly(as.numeric(year),1, raw=T) , data = Yield_Covariates_nna_plm, , index=c("comId", "year"), model="within", effect = "individual")
## Diagnostics
...
plot(residuals(FE_TavgPrecPetSmi_poly_GP_short_trend) ~ winterWheat - residuals(FE_TavgPrecPetSmi_poly_GP_short_trend) )
...
FE_TavgPrecPetSmi_poly_GP_short_time <-plm(formula = winterWheat ~  poly(Pet_GP_short_av, 2, raw=T) +   poly(Prec_GP_short_av, 2, raw=T) +  poly(Tavg_GP_short_av, 1, raw=T) + poly(SMI_GP_short_av, 2, raw=T)  + year , data = Yield_Covariates_nna_plm, , index=c("comId", "year"), model="within", effect = "individual")
## Diagnostics
...
RE_TavgPrecPetSmi_poly_GP_short_trend <- plm(formula = winterWheat ~    poly(Pet_GP_short_av, 2, raw=T) + poly(Prec_GP_short_av, 2, raw=T) +  poly(Tavg_GP_short_av, 1, raw=T) + poly(SMI_GP_short_av, 2, raw=T)  + poly(as.numeric(year),1, raw=T) , data = Yield_Covariates_nna_plm, , index=c("comId", "year"), model="random", effect = "individual")
## Diagnostics
...
hist(SMI_Jul*100)
## WinterWheat ##
hist(winterWheat) # looks pretty normal actually
hist(log(winterWheat)) # still normal but left skewed
## Control Variables ##
...
#######################################
## Polygons of SMI ##
formula_saturated   <- log(winterWheat) ~ poly(Prec_Jul,3, raw=T) + poly(Pet_Jul, 3, raw=T)  + poly(Tavg_Jul, 3, raw=T) + poly(SMI_Jul,3, raw=T) + poly(as.numeric(year))
formula_more        <- log(winterWheat) ~ poly(Prec_Jul,2, raw=T) + poly(Pet_Jul, 2, raw=T)  + poly(Tavg_Jul, 1, raw=T) + poly(SMI_Jul,2, raw=T) + poly(as.numeric(year))
formula_spline      <- log(winterWheat) ~ ns(Prec_Jul,2)          + ns(Pet_Jul, 2)           + ns(Tavg_Jul, 1)          + SMI_Jul_GDM            + poly(as.numeric(year))
formula_linear_inter<- log(winterWheat) ~ poly(Prec_Jul,1 ,raw=T)                            + poly(Tavg_Jul, 1, raw=T) + SMI_Jul                + poly(Tavg_Jul,1 ,raw=T):SMI_Jul + log(as.numeric(year))
formula_inter       <- log(winterWheat) ~ poly(Prec_Jul,3 ,raw=T) +  poly(Pet_Jul, 3, raw=T) + poly(Tavg_Jul, 3, raw=T) + poly(SMI_Jul,3, raw=T)  + poly(Tavg_Jul,3 ,raw=T):poly(SMI_Jul,3, raw=T) + log(as.numeric(year))
...
formula_step              <-  log(winterWheat) ~ poly(Prec_Jul,3 ,raw=T) + poly(Tavg_Jul, 3, raw=T) + poly(Pet_Jul, 3, raw=F) + SMI_Jul_GDM + log(as.numeric(year))
formula_step_ort          <-  log(winterWheat) ~ poly(Prec_Jul,2 ,raw=F) + poly(Pet_Jul, 8, raw=F) + poly(Tavg_Jul, 3, raw=F) + SMI_Jul_GDM + as.numeric(year)  
formula_step_noPet        <-  log(winterWheat) ~ poly(Prec_Jul,2, raw=T)                           + poly(Tavg_Jul, 2, raw=T) + SMI_Jul_GDM + as.numeric(year)
formula_step_spline       <-  log(winterWheat) ~ ns(Prec_Jul,5)     + ns(Pet_Jul, 15)         + ns(Tavg_Jul, 15)         + SMI_Jul_GDM + as.numeric(year)
formula_step_linear       <-  log(winterWheat) ~ Prec_Jul           + Pet_Jul                 + Tavg_Jul                 + SMI_Jul_GDM + as.numeric(year)
...
formula_step_inter        <-  log(winterWheat) ~ poly(Prec_Jul,3 ,raw=T) + poly(Tavg_Jul, 3, raw=T) + poly(Pet_Jul, 3, raw=F) + SMI_Jul_GDM  +
...
#########################################
GLM_LSDV_Jul_trend_poly_saturated <- glm(formula = log(winterWheat) ~ poly(Prec_Jul, 3, raw = T) + poly(Pet_Jul, 3, raw = T) + poly(as.numeric(year)) + comId,  data = Yield_Covariates_nna_plm)
LM_LSDV_Jul_trend_poly_saturated  <-  lm(formula = log(winterWheat) ~ poly(Prec_Jul, 3, raw = T) + poly(Pet_Jul, 3, raw = T) + poly(as.numeric(year)) + comId,  data = Yield_Covariates_nna_plm)
...
LSDV_Jul_trend_poly_dummy     <- lm(formula = log(winterWheat) ~ comId -1,         data = Yield_Covariates_nna_plm)
...
formula_PrecTavgSmiGDM <- log(winterWheat) ~ poly(Prec_Jul, container[r, 1], raw = T) +  poly(Tavg_Jul, container[r, 2], raw = T) + 
...
formula_PrecTavgSmiGDM_2v8poly <- log(winterWheat) ~ poly(Prec_Jul, container_2v8poly[r, 1], raw = T) +  poly(Tavg_Jul, container_2v8poly[r, 2], raw = T) + 
  dummy(SMI_Jul_GDM,c("severe drought","moderate drought","abnormal dry", "abnormal wet", "abundant wet","severe wet")) + log(as.integer(year)) + 
  dummy(comId)
...
for(r in 1:1){
 ## Approach with cv.lm ##   
#    glm.fit <- cv.lm(formula = log(winterWheat) ~ poly(Prec_Jul, container[r,1], raw = T) + poly(Pet_Jul, container[r,2], raw = T) + poly(Tavg_Jul, container[r,3], raw = T)+ comId +  log(as.numeric(year)),  data = Yield_Covariates_nna_plm)
#    lm.fit[r] <- cv.lm(Yield_Covariates_nna, log(winterWheat) ~ poly(Prec_Jul, container[r,1], raw = T) + poly(Pet_Jul, container[r,2], raw = T) + poly(Tavg_Jul, container[r,3], raw = T) +  comId +  log(as.numeric(year)) - 1, 10)
...
r <- 3
container_2v8poly[r, ]
formula_PrecTavgSmiGDM <- log(winterWheat) ~ poly(Prec_Jul, 1, raw=T) +   poly(Tavg_Jul, 3, raw=T) + dummy(SMI_Jul_GDM, c("severe drought", "moderate drought", "abnormal dry", "abnormal wet", 
...
'
glm model mit intercept liefert gleiche Ergebnisse wie pln, wobei intercept dann comID1001 ist, wenn Datensatz plm.data Format hat.
glm model mit data format liefert nur ähnliche, aber vergleichbare Ergebnisse.
In meinen Augen hat das mit leicht unterschiedlichen dummy Konfigurationen zu tun.
...
formula_PrecTavgSmiGDM_comId <- log(winterWheat) ~ poly(Prec_Jul, 1, raw = T) + poly(Tavg_Jul, 3, raw = T) + 
  dummy(SMI_Jul_GDM, c("severe drought", "moderate drought", "abnormal dry", "abnormal wet", "abundant wet", "severe wet")) + 
  log(as.integer(year)) + comId -1
class(Yield_Covariates_nna_plm$comId)

glm.fit3 <- glm(formula = formula_PrecTavgSmiGDM_comId,  data = Yield_Covariates_nna_plm)
...
plot(Prec_Jul, winterWheat,col="darkgrey")
...
#######################################
## Polygons of SMI ##
formula_saturated   <- log(winterWheat) ~ poly(Prec_Jun,3, raw=T) + poly(Pet_Jun, 3, raw=T) + poly(Tavg_Jun, 3, raw=T) + poly(SMI_Jun,3, raw=T) + poly(as.numeric(year))
formula_more        <- log(winterWheat) ~ poly(Prec_Jun,2, raw=T) + poly(Pet_Jun, 2, raw=T) + poly(Tavg_Jun, 1, raw=T) + poly(SMI_Jun,2, raw=T) + poly(as.numeric(year))
formula_spline      <- log(winterWheat) ~ ns(Prec_Jun,2)          + ns(Pet_Jun, 2)          + ns(Tavg_Jun, 1)          + SMI_Jun_GDM            + poly(as.numeric(year))
...
## Stepwise Function of SMI ##
formula_step              <-  log(winterWheat) ~ poly(Prec_Jun,3 ,raw=T) + poly(Pet_Jun, 3, raw=T) + poly(Tavg_Jun, 3, raw=T) + SMI_Jun_GDM + as.numeric(year) 
formula_step_ort          <-  log(winterWheat) ~ poly(Prec_Jun,2 ,raw=F) + poly(Pet_Jun, 2, raw=F) + poly(Tavg_Jun, 2, raw=F) + SMI_Jun_GDM + as.numeric(year)  
formula_step_noPet        <-  log(winterWheat) ~ poly(Prec_Jun,3, raw=T)                           + poly(Tavg_Jun, 3, raw=T) + SMI_Jun_GDM + as.numeric(year)
formula_step_spline       <-  log(winterWheat) ~ ns(Prec_Jun,10)         + ns(Pet_Jun, 15)         + ns(Tavg_Jun, 15)         + SMI_Jun_GDM + as.numeric(year)
formula_step_linear       <-  log(winterWheat) ~ Prec_Jun                + Pet_Jun                 + Tavg_Jun                 + SMI_Jun_GDM + as.numeric(year)
formula_step_nested       <-  log(winterWheat) ~ poly(Prec_Jun,2 ,raw=T) + poly(Pet_Jun, 2, raw=T) + poly(Tavg_Jun, 2, raw=T) + SMI_Jun_GDM + as.numeric(year)
...
#######################################
## Polygons of SMI ##
formula_saturated   <- log(winterWheat) ~ poly(Prec_Mai,3, raw=T) + poly(Pet_Mai, 3, raw=T) + poly(Tavg_Mai, 3, raw=T) + poly(SMI_Mai,3, raw=T) + poly(as.numeric(year))
formula_more        <- log(winterWheat) ~ poly(Prec_Mai,2, raw=T) + poly(Pet_Mai, 2, raw=T) + poly(Tavg_Mai, 1, raw=T) + poly(SMI_Mai,2, raw=T) + poly(as.numeric(year))
formula_spline      <- log(winterWheat) ~ ns(Prec_Mai,2)          + ns(Pet_Mai, 2)          + ns(Tavg_Mai, 1)          + SMI_Mai_GDM            + poly(as.numeric(year))
...
## Stepwise Function of SMI ##
formula_step              <-  log(winterWheat) ~ poly(Prec_Mai,3 ,raw=T) + poly(Pet_Mai, 3, raw=T) + poly(Tavg_Mai, 3, raw=T) + SMI_Mai_GDM + as.numeric(year) 
formula_step_ort          <-  log(winterWheat) ~ poly(Prec_Mai,2 ,raw=F) + poly(Pet_Mai, 1, raw=F) + poly(Tavg_Mai, 1, raw=F) + SMI_Mai_GDM + as.numeric(year)  
formula_step_noPet        <-  log(winterWheat) ~ poly(Prec_Mai,3, raw=T)                           + poly(Tavg_Mai, 3, raw=T) + SMI_Mai_GDM + as.numeric(year)
formula_step_spline       <-  log(winterWheat) ~ ns(Prec_Mai,15)         + ns(Pet_Mai, 15)         + ns(Tavg_Mai, 15)         + SMI_Mai_GDM + as.numeric(year)
formula_step_linear       <-  log(winterWheat) ~ Prec_Mai                + Pet_Mai                 + Tavg_Mai                 + SMI_Mai_GDM + as.numeric(year)
formula_step_nested       <-  log(winterWheat) ~ poly(Prec_Mai,2 ,raw=T) + poly(Pet_Mai, 1, raw=T) + poly(Tavg_Mai, 1, raw=T) + SMI_Mai_GDM + as.numeric(year)
...
#######################################
## Polygons of SMI ##
formula_saturated   <- log(winterWheat) ~ poly(Prec_Apr,3, raw=T) + poly(Pet_Apr, 3, raw=T) + poly(Tavg_Apr, 3, raw=T) + poly(SMI_Apr,3, raw=T) + poly(as.numeric(year))
formula_more        <- log(winterWheat) ~ poly(Prec_Apr,2, raw=T) + poly(Pet_Apr, 2, raw=T) + poly(Tavg_Apr, 1, raw=T) + poly(SMI_Apr,2, raw=T) + poly(as.numeric(year))
formula_spline      <- log(winterWheat) ~ ns(Prec_Apr,2)          + ns(Pet_Apr, 2)          + ns(Tavg_Apr, 1)          + SMI_Apr_GDM            + poly(as.numeric(year))
...
## Stepwise Function of SMI ##
formula_step              <-  log(winterWheat) ~ poly(Prec_Apr,3 ,raw=T) + poly(Pet_Apr, 3, raw=T) + poly(Tavg_Apr, 3, raw=T) + SMI_Apr_GDM + as.numeric(year) 
formula_step_ort          <-  log(winterWheat) ~ poly(Prec_Apr,2 ,raw=F) + poly(Pet_Apr, 1, raw=F) + poly(Tavg_Apr, 1, raw=F) + SMI_Apr_GDM + as.numeric(year)  
formula_step_noPet        <-  log(winterWheat) ~ poly(Prec_Apr,3, raw=T)                           + poly(Tavg_Apr, 3, raw=T) + SMI_Apr_GDM + as.numeric(year)
formula_step_spline       <-  log(winterWheat) ~ ns(Prec_Apr,15)         + ns(Pet_Apr, 15)         + ns(Tavg_Apr, 15)         + SMI_Apr_GDM + as.numeric(year)
formula_step_linear       <-  log(winterWheat) ~ Prec_Apr                + Pet_Apr                 + Tavg_Apr                 + SMI_Apr_GDM + as.numeric(year)
formula_step_nested       <-  log(winterWheat) ~ poly(Prec_Apr,2 ,raw=T) + poly(Pet_Apr, 1, raw=T) + poly(Tavg_Apr, 1, raw=T) + SMI_Apr_GDM + as.numeric(year)
...
#######################################
## Polygons of SMI ##
formula_saturated   <- log(winterWheat) ~ poly(Prec_Mar,3, raw=T) + poly(Pet_Mar, 3, raw=T) + poly(Tavg_Mar, 3, raw=T) + poly(SMI_Mar,3, raw=T) + poly(as.numeric(year))
formula_more        <- log(winterWheat) ~ poly(Prec_Mar,2, raw=T) + poly(Pet_Mar, 2, raw=T) + poly(Tavg_Mar, 1, raw=T) + poly(SMI_Mar,2, raw=T) + poly(as.numeric(year))
formula_spline      <- log(winterWheat) ~ ns(Prec_Mar,2)          + ns(Pet_Mar, 2)          + ns(Tavg_Mar, 1)          + SMI_Mar_GDM            + poly(as.numeric(year))
...
## Stepwise Function of SMI ##
formula_step              <-  log(winterWheat) ~ poly(Prec_Mar,3 ,raw=T) + poly(Pet_Mar, 3, raw=T) + poly(Tavg_Mar, 3, raw=T) + SMI_Mar_GDM + as.numeric(year) 
formula_step_ort          <-  log(winterWheat) ~ poly(Prec_Mar,3 ,raw=F) + poly(Pet_Mar, 3, raw=F) + poly(Tavg_Mar, 3, raw=F) + SMI_Mar_GDM + as.numeric(year)  
formula_step_noPet        <-  log(winterWheat) ~ poly(Prec_Mar,3, raw=T)                           + poly(Tavg_Mar, 3, raw=T) + SMI_Mar_GDM + as.numeric(year)
formula_step_spline       <-  log(winterWheat) ~ ns(Prec_Mar,15)         + ns(Pet_Mar, 15)         + ns(Tavg_Mar, 15)         + SMI_Mar_GDM + as.numeric(year)
formula_step_linear       <-  log(winterWheat) ~ Prec_Mar                + Pet_Mar                 + Tavg_Mar                 + SMI_Mar_GDM + as.numeric(year)
formula_step_nested       <-  log(winterWheat) ~ poly(Prec_Mar,2 ,raw=T) + poly(Pet_Mar, 1, raw=T) + poly(Tavg_Mar, 1, raw=T) + SMI_Mar_GDM + as.numeric(year)
...
table(upper_Feb)
lower_Feb==upper_Feb
Yield_Covariates_nna_plm$log(winterWheat)_zscore <-scale(log(winterWheat), center = TRUE, scale = TRUE)
table(log(winterWheat)_zscore)
...
## PLM - time demeaning, individuual ##
formula_saturated <- log(winterWheat) ~ poly(Prec_Feb,3, raw=T) + poly(Pet_Feb, 3, raw=T) + poly(Tavg_Feb, 3, raw=T) + poly(SMI_Feb,3, raw=T) + poly(as.numeric(year))
formula_more <- log(winterWheat) ~      poly(Prec_Feb,2, raw=T) + poly(Pet_Feb, 2, raw=T) +  poly(Tavg_Feb, 1, raw=T) + poly(SMI_Feb,1, raw=T) + poly(as.numeric(year))
formula <- log(winterWheat) ~           poly(Prec_Feb,1, raw=T) +  poly(Tavg_Feb, 2, raw=T) + lower_Feb + upper_Feb + as.numeric(year)

formula_step <-  log(winterWheat) ~      poly(Prec_Feb,1, raw=T) +  poly(Pet_Feb, 1, raw=T) + poly(Tavg_Feb, 1, raw=T) + SMI_Feb_GDM + as.numeric(year)    

formula_step_noPet <-  log(winterWheat) ~      poly(Prec_Feb,1, raw=T) +  poly(Tavg_Feb, 2, raw=T) + SMI_Feb_GDM + as.numeric(year)   
...
pairs( log(winterWheat) ~ poly(Prec_Feb,1, raw=T) + poly(Pet_Feb, 1, raw=T) +  poly(Tavg_Feb, 1, raw=T) + poly(SMI_Feb,1, raw=T) + poly(as.numeric(year)), Yield_Covariates_nna_plm)
...
plot(log(winterWheat),SMI_Feb, data= Yield_Covariates_nna_plm)
