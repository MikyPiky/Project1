...
getwd()
setwd("E://Documents//Shared//projects//correlation")
...
image(mSMiRas)
plot(mSMiRas, c(700:720)) #ohne Angabe der Layers würde hier automatisch die erste 16 geplottet werden
nlayers(mSMiRas)
...
mSMiRasM<-setZ(mSMiRas, idx) #hier füge ich eine besser lesbare Zeitdimension hinzu
...
library(colorspace)
myTheme=rasterTheme(region=sequential_hcl(n=100, , power=1)) #eigenes Farbtheme using sequential palette fronm colorspace
...
spplot(KreismSMiExAvg)


new_frame=cbind(matrix(as.vector(coordinates(KreisPol)),ncol=2),KreismSMiExAvg[,"X640"])
colnames(new_frame)=c("x","y","z")
dataframe=data.frame(new_frame)
str(dataframe)
...
spplot(dataframe, add=TRUE)
levelplot(KreismSMiExAvg[, 640])


spplot(KreismSMiExAvg@data) #wahrscheinlich zu viele Dimensionen

plot(KreismSMiExAvg@data$X540)
KreismSMiExAvg@data$SHAPE_AREA
KreismSMiExAvg
KreismSMiExAvg[,c("USE","RS","GEN","SHAPE_LENG", "SHAPE_AREA","X540")]
spplot(KreismSMiExAvg[,c("X640", "X652")])
